...
| target | 否 | 打开的容器类型：   - 移动端：    - **fullScreen**（默认**）**：全屏容器打开   - **panel**：[沉浸式容器](1428-immersive-container-1.md)打开 - 桌面端：    - **workbench（默认）**：桌面端工作台打开   - **popupWindow**：独立窗口打开   - **panel**：第四栏打开   - **slide**：侧边栏打开 |
