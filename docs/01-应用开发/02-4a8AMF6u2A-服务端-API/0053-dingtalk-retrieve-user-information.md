...
> Path: 应用开发 / 服务端 API / 通讯录管理 > 获取用户通讯录个人信息
...
企业内部应用和第三方企业应用在调用该接口前，需要先获取个人用户的accessToken，获取方式请参考[获取登录用户的访问凭证](0028-obtain-identity-credentials.md)。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用服务端接口的授权凭证。使用个人用户的accessToken，请参考[获取登录用户的访问凭证](0028-obtain-identity-credentials.md)。 |
