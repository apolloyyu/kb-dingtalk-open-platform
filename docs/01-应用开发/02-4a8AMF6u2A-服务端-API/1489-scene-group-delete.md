...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 即时通信 > 会话管理 > 场景群 > 删除群成员
> Updated: 2026-08-25 09:37:18
...
> - 未接入的开发者建议使用新版[删除场景群成员](0750-api-removescenegroupmember.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | be311\*\*\*\*\* | 调用该API的应用凭证。   - 企业内部应用，通过[获取企业内部应用的access\_token](1446-obtain-orgapp-token.md)接口获取。 - 第三方企业应用，通过[获取第三方企业的access\_token](1448-obtain-isvapp-token.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| open\_conversation\_id | String | 是 | cid15\*\*\*\*\*== | 群ID，调用[创建群](1486-create-a-scene-group-v2.md)接口获取`open_conversation_id`参数值。 |
...
| 400020 | 没有访问权限 | 请在调用[创建群](1486-create-a-scene-group-v2.md)接口后，延迟几秒再调用本接口 |
