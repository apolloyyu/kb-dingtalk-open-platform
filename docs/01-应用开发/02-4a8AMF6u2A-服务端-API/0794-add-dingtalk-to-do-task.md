...
> - 如果尚未使用本接口创建个人待办（原钉钉官方待办），请通过[创建钉钉个人待办任务](0795-api-createpersonaltodotask.md)接口进行创建。
> - 如果已使用本接口创建个人待办（原钉钉官方待办），你需要于 2024 年 1 月 31 日之前尽快将创建个人待办任务从本接口迁移到[创建钉钉个人待办任务](0795-api-createpersonaltodotask.md)接口，若未完成迁移，后续调用本接口且未填写 detailUrl 字段的入参，则会出现拦截报错提示。
