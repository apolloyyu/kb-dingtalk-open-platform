...
> Path: 应用开发 / 服务端 API / 即时通信 > 互动卡片 > 卡片平台模板复制
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证，第三方企业应用通过[获取第三方应用授权企业的accessToken](0033-obtain-the-access-token-of-the-authorized-enterprise-1.md)接口获取。 |
