...
> Path: 应用开发 / 服务端 API / 音视频 > 会议 > 停止视频会议直播推流
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证：   - 企业内部应用可调用[获取企业内部应用的accessToken](0032-obtain-the-access-token-of-an-internal-app.md)接口获取。 - 第三方企业应用可调用[获取第三方应用授权企业的accessToken](0033-obtain-the-access-token-of-the-authorized-enterprise-1.md)接口获取。 - 第三方个人应用可调用[获取第三方个人应用的access\_token](0034-obtain-personal-application.md)接口获取。 |

### 路径参数

| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| conferenceId | String | 是 | 会议id，可调用[创建视频会议](0398-create-a-video-conference.md)接口获取返回参数`conferenceId`字段。 |

### 请求体

| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| streamId | String | 是 | 推流id，可调用[开启视频会议直播推流](0411-video-conference-enables-live-stream-ingest.md)接口获取返回参数`successStreamMap`字段。 |
