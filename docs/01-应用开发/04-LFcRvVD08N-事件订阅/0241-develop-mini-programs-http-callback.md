...
3. 在弹出的页面，填写基本信息，然后单击**保存**。

   ![create ](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1704438871/p259097.png)
4. 在应用详情页面，单击**添加应用能力**，然后选择**小程序**即可。

   ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1704438871/p1099459.png)
...
2. 在[开发者后台](https://open-dev.dingtalk.com/)，单击已创建的第三方小程序应用，完成以下配置：
   ...
   2. 打开已下载的代码示例的`eapp-isv-quick-start-java/src/main/java/com/config/Constant.java`文件，提供以下配置：
      ...
      2. 在事件订阅中，选择推送方式为SyncHTTP推送方式，并获取**TOKEN**和数据加解密密钥**AES\_KEY**。

         ![p166111](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1704438871/p256401.png)
   3. 启动后端服务：
...
5. 接收到授权开通事件后，需要依次调用以下三个接口完成应用激活，激活完成后才算授权成功：
   ...
   1. 调用[获取第三方企业应用的suite\_access\_token](../02-4a8AMF6u2A-服务端-API/1444-obtain-application-suite-ticket.md)接口获取第三方应用凭证（suite\_access\_token）。
   2. 调用[获取授权企业的永久授权码](../02-4a8AMF6u2A-服务端-API/1867-obtain-a-permanent-authorization-code.md)接口获取永久授权码。
   3. 调用[激活应用](../02-4a8AMF6u2A-服务端-API/1860-activate-suite.md)接口激活企业授权的应用。
...
6. 登录[开发者后台](https://open-dev.dingtalk.com/#/appMgr/provider/eapp/54716/6)，进入应用详情，并单击**版本管理与发布**，再点击**灰度**或**发布**。
