...
doc_id: "8s4gF3RsZI"
updated_at: "2026-08-25 09:39:34"
---

> Source: https://open.dingtalk.com/document/development/query-form-instances-using-advanced-search-conditions
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 宜搭 > 表单 > 通过高级查询条件获取表单实例数据（包括子表单组件数据）
> Updated: 2026-08-25 09:39:34
...
> - 未接入的开发者建议使用新版[通过高级查询条件获取表单实例数据（包括子表单组件数据）](0336-api-searchformdatasecondgeneration-v2.md)接口，已接入用户不受影响。
...
| searchCondition | String | 否 | 用于检索表单实例数据的检索条件，格式参考[表单筛选组件格式](0366-instructions-for-creating-or-updating-form-data-format.md#d902d581d1k3s)。 |
...
| orderConfigJson | String | 否 | 排序规则，详情参考[宜搭数据格式和搜索结果排序说明](0366-instructions-for-creating-or-updating-form-data-format.md)。 |
...
| formData | Map | 表单实例数据以Map结构展示，参考[表单实例FormData结构](0366-instructions-for-creating-or-updating-form-data-format.md#fd1baaed40ddo)。 |
