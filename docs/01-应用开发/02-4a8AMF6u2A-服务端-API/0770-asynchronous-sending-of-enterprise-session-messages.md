...
> Path: 应用开发 / 服务端 API / 即时通信 > 消息通知 > 工作通知 > 发送工作通知
...
工作通知消息是以某个应用的名义推送到员工的工作通知消息，例如生日祝福、入职提醒等。可以发送文本、语音、链接等，消息类型和样例可参考[消息通知类型](0776-message-types-and-data-format.md)。

> **[!NOTE]**
>
> 如果接口发送成功，接收人没有收到信息，可调用[获取工作通知消息的发送结果](0774-gets-the-result-of-sending-messages-asynchronously-to-the-enterprise.md)查询结果，并对比文档中的返回错误码。
...
> **[!IMPORTANT]**
>
> 2020年11月27日以后创建的第三方企业应用，请调用[使用模板发送工作通知消息](0775-work-notification-templating-send-notification-interface.md)接口发送工作通知。

- 适用场景：适用于发送通知类的消息，不包含审批任务等。
- 不适用场景：需要发送一条任务类的通知提醒给员工，比如审批任务等，调用此接口无法实现。这种场景可以使用[创建待办](0794-add-dingtalk-to-do-task.md)接口。
...
| msg | JSON Object | 是 | {  "msgtype": "text",  "text": {  "content": "请提交日报。"  }  } | 消息内容，最长不超过2048个字节，支持以下[消息通知类型](0776-message-types-and-data-format.md)，msgtype 包括：  **[!IMPORTANT]**  发送消息时，不支持同时发送多种消息类型。   - text：文本消息 - image：图片消息 - voice：语音消息 - file：文件消息 - link：链接消息 - oa：OA消息  **[!NOTE]**  OA消息支持通过`status_bar`参数设置消息状态文案和颜色，发送后可通过[更新工作通知状态栏](0772-update-work-notification-status-bar.md)接口更新消息状态和颜色。 - markdown：Markdown消息 - action\_card：卡片消息 |
...
| errmsg | String | ok | 返回码描述。  **[!IMPORTANT]**  如果接口发送成功，接收人没有收到信息，可调用[获取工作通知消息的发送结果](0774-gets-the-result-of-sending-messages-asynchronously-to-the-enterprise.md)查询结果，并对比文档中的返回错误码。 |
