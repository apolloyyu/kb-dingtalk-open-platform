...
调用服务端API-[创建用户](0055-user-information-creation.md)接口，实现用户创建并获取唯一标识`userid`：
...
- 指定部门：先调用[获取子部门ID列表](0083-obtain-the-list-of-sub-department-ids.md)接口获取部门ID：
...
根据用户`userid`[查询用户详情](0056-query-user-details.md)，用于数据同步后的校验或业务系统读取：
...
> **[!NOTE]**
>
> 部分字段的返回受权限限制，请参考[查询用户详情](0056-query-user-details.md)的返回字段说明。
...
根据用户`userid`，[更新用户信息](0057-user-information-update.md)：
...
根据用户`userid`从企业通讯录中[删除用户](0058-delete-a-user.md)：
...
- **Q2：调用创建/更新部门接口时，department\_id（部门ID）应如何获取和填写?**
  ...
  - 子部门ID：调用"[获取子部门ID列表](0083-obtain-the-list-of-sub-department-ids.md)"接口（`departmentListSubId`）获取指定父部门下的所有子部门ID
  - 多级部门：可通过递归调用获取深层子部门ID，或通过"[获取部门详情](0081-query-department-details0-v2.md)"接口查询特定部门的完整信息
