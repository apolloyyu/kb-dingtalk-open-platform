...
> Path: 应用开发 / 服务端 API / Teambition 项目管理 > 任务 > 项目任务 > 创建项目任务
...
| note | String | 否 | 任务备注。 |
| priority | Integer | 否 | 任务优先级，调用[查询优先级列表](1245-query-a-priority-list.md)接口获取priorityId值。 |
...
| id | String | 否 | 字段值id，如果该字段是成员字段，那么此处应该填写成员的在Teambition系统中的userId，获取Teambition用户id请参阅文档[根据userId获取Teambition项目用户ID](1257-obtain-dingtalk-teambition-user-id-based-on-userid.md)。 |
| thumbUrl | String | 否 | 如果该字段是成员字段，那么此处应该填写成员的头像地址。 |
| stageId | String | 否 | 任务列表ID，可调用[查询项目中的任务](1230-query-tasks-in-a-project.md)接口获取stageId。 |
| parentTaskId | String | 否 | 父任务id，可调用[查询项目中的任务](1230-query-tasks-in-a-project.md)接口获取taskId。 |
| scenariofieldconfigId | String | 否 | 任务类型id，任务类型比如：缺陷、需求，可调用[查询项目中的任务](1230-query-tasks-in-a-project.md)接口获取scenariofieldconfigId。 |
