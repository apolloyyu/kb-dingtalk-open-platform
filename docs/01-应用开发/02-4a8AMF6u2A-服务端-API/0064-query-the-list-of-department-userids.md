...
> Path: 应用开发 / 服务端 API / 通讯录管理 > 用户管理 > 获取部门用户userid列表
...
- **目前暂不支持一次性获取企业下所有员工userid值**，如果开发者希望获取企业下所有员工userid值，可以通过以下方法：
  ...
  - 调用[获取部门列表](0081-user-management-acquires-the-list-departments.md)接口，通过逐级遍历，获取该企业下在授权范围内的所有部门ID。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| dept\_id | Number | 是 | 100 | 部门deptId，可通过调用[获取部门列表](0081-user-management-acquires-the-list-departments.md)获取部门deptId。  **[!NOTE]**  如果是根部门，该参数传1。 |
