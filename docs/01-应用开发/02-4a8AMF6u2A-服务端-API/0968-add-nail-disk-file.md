...
> - 智能招聘相关文件的操作，都需要在智能招聘钉盘空间下操作才会生效，调用[钉盘-上传文件到钉盘](1565-add-file-and-folder.md)无法实现。

本接口使用步骤如下：

步骤一：调用[获取智能招聘文件上传信息](0967-obtain-information-about-the-dingtalk-disk-upload-file.md)口，获取文件上传OSS的临时凭证。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| mediaId | String | 是 | 文件mediaId，调用[获取智能招聘文件上传信息](0967-obtain-information-about-the-dingtalk-disk-upload-file.md)接口获取。    调用[获取智能招聘文件上传信息](0967-obtain-information-about-the-dingtalk-disk-upload-file.md)接口获取的mediaId，必须使用OSS上传后才能使用，上传流程请参考本文档介绍的接口调用流程示例。否则本接口会出现报错**钉盘空间不可用**。 |
