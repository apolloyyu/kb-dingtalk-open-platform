...
archived: false
method: POST
endpoint: https://api.dingtalk.com/v1.0/storage/spaces/{spaceId}/dentries/{dentryId}/copy
api_version: v2-new
app_types: 第三方企业应用
...
- dentryId (String, required): 需要被复制的文件或文件夹Id，可调用获取文件或文件夹列表或根据dentryUuid获取spaceId和dentryId接口获取id参数值。
...
- targetFolderId (String, required): 需要存放的位置父目录Id，可调用获取文件或文件夹列表接口获取id参数值。
