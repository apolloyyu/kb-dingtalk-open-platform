...
> Path: 应用开发 / 服务端 API / 更多开放 > 客户管理（官方CRM） > 使用教程 > CRM客户数据操作流程
...
步骤四：获取应用访问凭证[获取企业内部应用的accessToken](0032-obtain-the-access-token-of-an-internal-app.md)。

步骤五：调用服务端客户管理相关API。

1. 调用服务端API-[获取个人或企业客户的元数据](1344-obtain-the-metadata-of-individual-enterprise-customers.md)接口，获取客户管理客户元数据信息。
2. 客户管理创建客户数据：

   - 调用服务端API-[创建个人或企业客户数据](1345-add-crm-personal-customers.md)接口，进行客户数据的创建，获取客户数据`instanceId`。
   - 调用服务端API-[批量新增个人或企业客户数据](1348-add-multiple-relationship-data-in-batches.md)接口，进行客户数据的批量创建，获取多个客户数据`instanceId`。
3. 根据客户数据`instanceId`进行客户数据管理操作：

   - 根据客户数据`instanceId`，调用服务端API-[更新个人或企业客户数据](1346-update-crm-personal-customers.md)接口，进行更新客户数据。
   - 根据客户数据`instanceId`，调用服务端API-[批量更新个人或企业客户数据](1349-update-multiple-relational-data-tables-at-a-time.md)接口，进行更新客户数据。
   - 根据客户数据`instanceId`，调用服务端API-[删除个人或企业客户数据](1347-delete-crm-personal-customer.md)接口，进行删除客户数据。
   - 根据客户数据`instanceId`，调用服务端API-[批量获取个人或企业客户数据](1350-acquire-crm-individual-customers-in-batches.md)接口，进行批量获取客户数据信息。
4. 客户管理数据查询操作：

   - 调用服务端API-[获取全量个人或企业客户数据](1351-crm-obtains-all-private-sea-customer-data.md)接口，获取全部客户数据。
   - 调用服务端API-[根据指定条件查询个人或企业客户数据](1352-obtains-crm-individual-customers-in-batches-based-on-specified-query.md)接口，查询符合指定条件的客户数据。
   - 调用服务端API-[获取个人或企业客户查重字段](1353-obtain-duplicate-check-fields.md)接口，获取查重字段数据。
...
根据步骤二中的AppKey和AppSecret，获取应用访问凭证[获取企业内部应用的accessToken](0032-obtain-the-access-token-of-an-internal-app.md)。
...
1. 调用服务端API-[获取个人或企业客户的元数据](1344-obtain-the-metadata-of-individual-enterprise-customers.md)接口，获取个人或企业客户的元数据信息。
...
3. 根据客户数据instanceId进行客户数据管理操作：
   ...
   - 根据客户数据`instanceId`，调用服务端API-[更新个人或企业客户数据](1346-update-crm-personal-customers.md)接口，进行更新单个客户数据。
     ...
     ```
   - 根据客户数据`instanceId`，调用服务端API-[批量更新个人或企业客户数据](1349-update-multiple-relational-data-tables-at-a-time.md)接口，进行批量更新多个客户数据。
