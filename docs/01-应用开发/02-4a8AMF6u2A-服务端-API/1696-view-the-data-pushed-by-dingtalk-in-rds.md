...
6. 进入DMS后，即可查看数据库中存储的钉钉推送数据。
   ...
   > **[!NOTE]**
   >
   > 参考文档：具体回调数据字段说明，请参见[钉钉云推送数据格式](1697-deprecated-dingtalk-cloud-push-data-format.md)。
...
- `biz_type`：表示业务类型，不同数值对应不同的事件类型（如1表示组织变更，2表示用户变更等），具体取值请参考[钉钉云推送数据格式](1697-deprecated-dingtalk-cloud-push-data-format.md)。
