...
步骤一：调用[获取文件上传信息](0675-obtain-file-upload-informations.md)接口，获取上传文件需要的信息。
...
> **[!NOTE]**
>
> - 存储空间类型为USER时，空间拥有权限的用户都有操作权限，其他员工调用[添加权限](0682-add-permissions-file.md)接口进行授权。
> - 存储空间类型为APP时，任何人操作都需要调用[添加权限](0682-add-permissions-file.md)接口进行授权。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| uploadKey | String | 是 | 添加文件唯一标识，调用[获取文件上传信息](0675-obtain-file-upload-informations.md)接口获取uploadKey参数值。 |
