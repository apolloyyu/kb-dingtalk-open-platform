...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > OA审批 > 更新待办状态
...
> - 未接入的开发者建议使用新版[更新流程中心任务状态](0517-update-process-center-task-status.md)接口，已接入用户不受影响。
...
| process\_instance\_id | String | 是 | proc-zzz | 实例ID，由[创建实例](1514-initiate-an-approval-process-without-a-process.md)接口获取。 |
| tasks | TaskTopVo[] | 是 |  | 待办任务列表。 |
| task\_id | Number | 是 | 111 | 待办任务ID，需要在调用[查询待办列表](1521-query-a-user-s-to-do-items.md)接口时，主动设置该值。 |
