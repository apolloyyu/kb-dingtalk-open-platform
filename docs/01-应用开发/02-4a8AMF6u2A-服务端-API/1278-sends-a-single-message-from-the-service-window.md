...
- 目前支持的消息格式有文本、链接、卡片、markdown等，消息类型详见[消息类型介绍](1275-service-window-message-types-1.md)。
...
| image | Object | 否 | 图片信息。 |
| mediaId | String | 否 | 图片media\_id，可调用[上传媒体文件](0645-upload-media-files.md)接口获取。 |
| bizId | String | 否 | 可选参数，API调用方标识，仅用于定制调用方场景。 |
| accountId | String | 否 | 服务窗账号id，[自建服务窗应用](1276-self-built-service-window-application.md)调用时不需要传此参数。  非自建服务窗应用需要传此参数，此参数可以通过[获取企业下服务窗列表](1279-queries-the-list-of-services-under-an-enterprise.md)接口获得。 |
