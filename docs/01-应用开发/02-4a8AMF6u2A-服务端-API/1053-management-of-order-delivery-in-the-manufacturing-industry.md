...
> Path: 应用开发 / 服务端 API / 行业与生态 > 生态开放 > 金智CRM > 使用教程 > 制造业订单交付全过程管理
...
1. 调用服务端API-[获取部门用户基础信息](0065-queries-the-simple-information-of-a-department-user.md)接口，获取用户基础信息。
2. 调用服务端API-[客户资料](1054-add-or-edit-customer-profile.md)接口，进行客户信息的新增和编辑。
3. 调用服务端API-[产品信息](1062-add-or-edit-product-information.md)接口，进行产品信息的新增和编辑。
4. 调用服务端API-[销售机会](https://open.dingtalk.com/document/development/add-or-edit-opportunities)接口，进行销售机会的新增和编辑。
5. 调用服务端API-[报价记录](https://open.dingtalk.com/document/development/add-or-edit-quotation-records)接口，进行报价记录的新增和编辑。
6. 调用服务端API-[合同订单](1057-add-or-edit-contract-orders.md)接口，进行合同订单的新增和编辑。
7. 调用服务端API-[采购单](1060-edit-purchase-order.md)接口，进行采购单的新增和编辑。
8. 调用服务端API-[入库单](1063-add-or-edit-a-shipment-record.md)接口，进行入库单的新增和编辑。
9. 调用服务端API-[发货单](1058-add-or-edit-invoices.md)接口，进行发货单的新增和编辑。
10. 调用服务端API-[出库单](1064-add-or-edit-an-issue-ticket.md)接口，进行出库单的新增和编辑。

数据信息管理：

- 调用服务端API-[获取数据详情](1065-queries-data-details.md)接口，可以获取销售机会、报价单、订单、采购、入库、发货和出库一系列环节的数据详细信息。
- 调用服务端API-[获取数据列表](1066-obtain-the-data-list.md)接口，可得到某一单据的数据列表信息。
...
- 根据订单交付流程中msgId参数和dataType参数，调用服务端API-[获取数据详情](1065-queries-data-details.md)接口，可以获取销售机会、报价单、订单、采购、入库、发货和出库一系列环节的数据详细信息。
- 调用服务端API-[获取数据列表](1066-obtain-the-data-list.md)接口，可得到某一单据的数据列表信息。
