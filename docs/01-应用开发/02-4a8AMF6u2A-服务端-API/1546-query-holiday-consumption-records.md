...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 协同办公 > 考勤 > 批量查询员工假期余额变更记录
...
> - 未接入的开发者建议使用新版[批量查询员工假期余额变更记录](0239-batch-query-employee-leave-balance-change-record.md)接口，已接入用户不受影响。
...
| leave\_code | String | 是 | f84a2dxxxx | 假期类型唯一标识，通过[查询假期规则列表](0237-holiday-type-query.md)接口获取leave\_code参数值。 |
