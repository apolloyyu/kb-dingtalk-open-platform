...
> Path: 应用开发 / 服务端 API / 考勤 > 考勤组管理 > 创建考勤组
...
| id | Number | 否 | 123 | 班次ID，可通过[获取班次摘要信息](0202-enterprise-shift-query-in-batches.md)接口获取id参数值。 |
...
| default\_class\_id | Number | 否 | 1234 | 默认班次ID。  **[!NOTE]**  固定班制必填，可通过[获取班次摘要信息](0202-enterprise-shift-query-in-batches.md)接口获取id参数值。 |
...
| device\_id | Number | 否 | 1311089987 | 设备ID，调用[查询员工智能考勤机列表](0220-query-the-list-of-employee-intelligent-attendance-machines.md)获取deviceid参数值。 |
