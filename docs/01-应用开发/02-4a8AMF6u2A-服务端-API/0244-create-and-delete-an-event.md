...
> Path: 应用开发 / 服务端 API / 日程 > 使用教程 > 创建、修改、查询及删除日程
...
2. 根据日程`id`，调用服务端API-[修改日程](0251-modify-event.md)接口，进行日程信息修改。
3. 根据日程`id`，调用服务端API-[查询单个日程详情](0252-query-details-about-an-event.md)接口，获取日程详情信息。
4. 根据日程`id`，调用服务端API-[删除日程](0250-delete-event.md)接口，删除创建的日程。
