...
2. 调用接口获取授权数据
   ...
   可调用调用[获取服务窗联系人信息](1882-obtains-the-contact-information-of-the-service-window.md)接口获取授权之后的手机号和企业名称等数据。

## 获取全量已关注服务窗的联系人信息

自建服务窗应用首次启用时，可以使用[批量获取服务窗联系人数据](1883-obtain-contact-data-from-the-service-window.md)接口全量同步历史联系人数据。

增量联系人数据请通过监听关注、取消关注事件回调获取，而不是频繁调用该接口来进行同步，事件回调请参考[服务窗关注与取消回调](1884-callback-event-for-service-window-following-and-cancellation.md)。
