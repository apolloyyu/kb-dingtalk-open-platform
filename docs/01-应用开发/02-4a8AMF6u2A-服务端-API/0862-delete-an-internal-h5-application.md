...
> Path: 应用开发 / 服务端 API / 钉钉应用 > 应用管理 > 删除企业内部应用
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| opUnionId | String | 是 | 操作人的unionId，可调用[查询用户详情](0055-query-user-details.md)接口获取。      操作删除的员工必须满足是以下两种身份之一，才可以成功删除应用，否则接口会报错**不合法的agentId**。   - 该应用所在企业的创建者。 - 该应用的创建人。 |
