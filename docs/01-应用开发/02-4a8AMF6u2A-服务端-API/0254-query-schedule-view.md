...
> Path: 应用开发 / 服务端 API / 日程 > 日程 > 查询日程视图
...
- 本接口与**查询日程列表**接口的区别在于对循环日程的处理。本接口会根据循环规则展开查询区间内的所有循环日程，但不会实际在数据库中生成对应实例。 例如，有一个从1.1号开始的每日循环日程，查询时已经是1.4号，查询区间为1.1号至1.10号，那么本接口会返回1.1～1.10号的所有循环日程，而[查询日程列表](0253-query-an-event-list.md)接口是不会返回1.4号之后的日程。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| userId | String | 是 | 要查询的用户unionId。   - 企业内部应用和第三方企业应用，调用[查询用户详情](0055-query-user-details.md)接口获取unionid参数值。 - 第三方个人应用，调用[获取用户通讯录个人信息](0053-dingtalk-retrieve-user-information.md)接口获取unionId参数值。 |
| calendarId | String | 是 | 日程所属的日历ID，调用[查询日历](0260-query-a-calendar.md)接口查询当前用户日程，使用**primary**关键字，表示当前用户的主日历。 |
...
| maxAttendees | Integer | 否 | 每个日程的参与者查询个数，默认100，最大100。 如果参会人数超过100人，需要拉取全部参会人请使用[获取日程参与者](0258-get-the-participants-of-a-schedule.md)接口获取。 |
