...
4. 单击**添加**，配置机器人信息：
   ...
   | 机器人名字 | 添加机器人名称 |
   | 安全设置 | 安全设置类型：  - 自定义关键词 - 加签 - IP 地址（段） 安全设置详情参考[自定义机器人安全设置](0084-customize-robot-security-settings.md)。 |
   | （可选）是否开启 Outgoing 机制 | 通过 @ 群机器人，将消息发送到指定外部服务，还可以将外部服务的响应结果返回到群聊会话。  机器人接收消息类型和数据格式，详情参考[消息发送与接收类型](../02-4a8AMF6u2A-服务端-API/0700-robot-message-type.md)。 |
...
- [自定义机器人发送群聊消息](../02-4a8AMF6u2A-服务端-API/0703-custom-bot-to-send-group-chat-messages.md)
