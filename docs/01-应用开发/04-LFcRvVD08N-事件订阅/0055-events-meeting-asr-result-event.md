...
doc_id: "fbr7ixmOBM"
updated_at: "2025-08-27 16:11:20"
---
...
- `data.openConfModel.scheduleConferenceId`（string）：预约会议id。
- `data.payload`（object）：转写结果内容。
- `data.payload.words`（array，必填）：识别单词列表。
- `data.payload.words[].startTime`（long，必填）：开始时间。
- `data.payload.words[].text`（string，必填）：单词内容。
- `data.payload.words[].endTime`（long，必填）：结束时间。
...
- `data.bizType`（string）：转写事件对应业务类型。
- `data.recordId`（string）：转写事件对应。业务id
...
- `openConfModel.scheduleConferenceId`（string）：预约会议id。
- `payload`（object，必填）：转写结果内容。
- `payload.words`（array，必填）：识别单词列表。
- `payload.words[].startTime`（long，必填）：开始时间。
- `payload.words[].text`（string，必填）：单词内容。
- `payload.words[].endTime`（long，必填）：结束时间。
...
- `bizType`（string，必填）：转写事件对应业务类型。
- `recordId`（string，必填）：转写事件对应。业务id
...
- `biz_data.openConfModel.scheduleConferenceId`（string）：预约会议id。
- `biz_data.payload`（object）：转写结果内容。
- `biz_data.payload.words`（array，必填）：识别单词列表。
- `biz_data.payload.words[].startTime`（long，必填）：开始时间。
- `biz_data.payload.words[].text`（string，必填）：单词内容。
- `biz_data.payload.words[].endTime`（long，必填）：结束时间。
...
- `biz_data.bizType`（string）：转写事件对应业务类型。
- `biz_data.recordId`（string）：转写事件对应。业务id
