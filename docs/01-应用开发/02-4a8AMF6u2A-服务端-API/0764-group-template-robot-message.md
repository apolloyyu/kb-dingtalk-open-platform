...
支持场景：基于群模板创建的群，详情参见[创建场景群](0747-create-a-scene-group.md)。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| target\_open\_conversation\_id | String | 是 | cid9FTRQSLo+sK\*\*\*\*\*== | 群ID，调用[创建场景群](0747-create-a-scene-group.md)接口获取`open_conversation_id`参数值。 |
...
| open\_msg\_id | String | 36\*\*\*\*\* | processQueryKey，可用此key调用[查询企业机器人群聊消息用户已读状态](0723-chatbot-queries-the-read-status-of-a-message.md)接口获取消息已读状态。 |
