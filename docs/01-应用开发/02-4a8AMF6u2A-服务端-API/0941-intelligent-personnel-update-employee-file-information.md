...
| field\_code | String | 否 | sys01-birthTime | 更新的字段code，调用[获取花名册元数据](0938-intelligent-personnel-roster-metadata-query.md)接口获取field\_code参数值。  **[!NOTE]**   - sys00分组内字段code值为汉字，如姓名，更新时直接传入**"姓名"**作为该参数值。 - sys00分组内的字段，**部门**和**直属主管**暂不支持接口更新。 |
...
| group\_id | String | 否 | sys01 | 分组标识，调用[获取花名册元数据](0938-intelligent-personnel-roster-metadata-query.md)接口获取group\_id参数值。 |
