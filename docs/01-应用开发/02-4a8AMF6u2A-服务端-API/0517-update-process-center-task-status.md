...
> Path: 应用开发 / 服务端 API / OA 审批 > 自有 OA 审批 > 流程中心任务 > 更新流程中心任务状态
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| processInstanceId | String | 否 | OA审批流程实例ID，调用[创建实例](0512-create-a-ticket-approval-instance.md)接口获取`processInstanceId`参数值。 |
| tasks | Array | 是 | OA审批任务列表，最多20个数。 |
| taskId | Long | 是 | OA审批任务ID，可调用[查询通过流程中心集成的OA审批任务](0516-query-oa-approval-tasks-integrated-through-process-center.md)接口获取taskId参数值。 |
