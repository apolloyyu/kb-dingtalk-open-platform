...
- 在使用HTTP调用时，必须设置**signature**参数，钉钉会对请求进行签名验证，以保证安全。签名计算方式可参考[个人免登场景的签名计算方法](1429-signature-personal-registration.md)。
...
| signature | String | 是 | ddsdssfsdfxxxx | 通过appSecret计算出来的签名值，计算方式请参考[个人免登场景的签名计算方法](1429-signature-personal-registration.md)。  **[!NOTE]**  使用SDK调用时，这个值不需要填写，SDK内部已做处理。 |
