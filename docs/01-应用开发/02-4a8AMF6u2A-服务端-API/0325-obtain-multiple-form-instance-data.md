...
> Path: 应用开发 / 服务端 API / 宜搭 > 表单 > 批量获取表单实例数据
> Updated: 2026-06-03 10:11:48
...
| formInstanceIdList | Array of String | 是 | 宜搭表单实例Id，调用[获取实例ID列表](0309-api-getinstanceidlist-v2.md)接口或者[获取多个表单实例ID](0323-api-searchformdataidlist-v2.md)接口获取。 |
| needFormInstanceValue | Boolean | 否 | 是否需要宜搭表单组件格式的实例数据。   - **true**：需要 - **false**：不需要 |
| userId | String | 是 | 用户userId，可通过[获取部门用户userid列表](0064-query-the-list-of-department-userids.md)接口获取。 |
