...
doc_id: "zuIUrvCZcM"
updated_at: "2026-08-25 09:39:00"
---

> Source: https://open.dingtalk.com/document/development/submit-documents
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 文档/文件 > 存储管理 > 文件传输 > 提交文件
> Updated: 2026-08-25 09:39:00
...
> - 未接入的开发者建议使用新版[提交文件](0675-submittal-file.md)接口，已接入用户不受影响。

### 接口调用步骤

步骤一：调用[获取文件上传信息](1599-obtain-storage-upload-information.md)接口，获取上传文件需要的信息。
...
>
> 可调用[添加权限](1595-add-storage-permissions.md)接口进行授权。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| unionId | String | 是 | 操作者unionId，调用[查询用户详情](0055-query-user-details.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| uploadKey | String | 是 | 添加文件唯一标识，调用[获取文件上传信息](1599-obtain-storage-upload-information.md)接口获取uploadKey参数值。 |
| name | String | 是 | 文件的名称，带后缀。命名有以下要求：   - 头尾不能包含空格，否则会自动去除 - 不能包含特殊字符，包括：制表符、\*、"、<、>、| - 不能以"."结尾 |
| parentId | String | 是 | 父目录Id。根目录时，该参数是0，可调用[获取文件或文件夹列表](0665-get-a-list-of-files-or-folders.md)接口获取parentId参数值。 |
