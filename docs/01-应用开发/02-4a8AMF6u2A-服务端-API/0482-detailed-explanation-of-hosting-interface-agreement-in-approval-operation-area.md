...
- **CustomActionDefinition 模型解析**
  ...
  | ∟schema | json Array  [{  componentName: 'TextareaField',  **props: {**  **id: 'comment',**  **},**  }] | 弹窗中的表单定义组件列表（behavior为modal类型时需配置该值） | 参考[保存流程中心外部集成审批模板](0541-api-premiumsaveintegratedprocess.md)中的【**FormComponent参数补充说明**】  - **TextField**：单行输入框 - **TextareaField**：多行输入框 - **DDSelectField**：单选框 - **DDMultiSelectField**：多选框 - **TextNote**：文字说明控件 - **DDPhotoField**：图片控件 - **DDAttachment**：附件 - **InnerContactField**：联系人控件    - users string[] staffId列表 |
  ...
  | ∟value | string | 控件值 | 参考[保存流程中心外部集成审批模板](0541-api-premiumsaveintegratedprocess.md)中的示例 |
  ...
  - **弹窗中的表单定义组件列表（behavior为modal类型时需配置该值）**
    ...
    参考[创建或更新审批模板](0509-create-orupdate-the-approval-template-new.md)中的【**FormComponent参数补充说明**】，当前支持以下表单组件类型：
...
**同步审批模板数据到钉钉：**调用新版服务端API-[保存流程中心外部集成审批模板](0541-api-premiumsaveintegratedprocess.md)，获取模板的唯一编码`processCode`。
...
**同步审批实例数据到钉钉**：根据模板编码`processCode`，调用新版服务端API-[保存流程中心外部集成审批实例](0542-api-premiumexternalintegrationprocessinstance.md)发起审批实例，获取审批实例`processInstanceId`。
...
**同步审批任务数据到钉钉：**根据审批实例`processInstanceId`和待办事项列表tasks，调用[保存流程中心外部集成审批任务](0543-api-premiumsaveintegratedtask.md)，可以将三方系统内的审批节点信息同步到钉钉OA审批，获取待办事项的taskId并生成对应的钉钉待办任务。
...
**自定义同意/拒绝审批任务：**审批人从钉钉端打开自定义审批页进行审批时，操作同意/拒绝等功能模块，将会回调[保存流程中心外部集成审批模板](0541-api-premiumsaveintegratedprocess.md)接口中`CUSTOM_ACTION_APPLY`模块中配置的回调接口，三方在执行自身业务逻辑完之后，需主动同步任务状态至钉钉。根据审批实例`processInstanceId`和审批待办任务taskId，可以调用[更新流程中心任务状态](0517-update-process-center-task-status.md)接口，同步完成自有审批待办状态的更新。

在或签等场景，可以调用[批量取消流程中心待处理任务](0518-cancel-multiple-oa-approval-tasks.md)接口，批量将审批实例下正在运行中的待办事项设置为CANCELED。
