...
> Path: 应用开发 / 服务端 API / OA 审批 > 高级版专享接口 > 官方 OA 审批 > 审批实例 > 加签审批任务
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| taskId | Long | 是 | 审批任务ID。可通过调用[获取单个审批实例详情](0497-obtains-the-details-of-a-single-approval-instance-pop.md)接口获取`taskId`参数值。 |
