...
> - 未接入的开发者建议使用新版[获取企业所有应用列表](0863-obtains-a-list-of-all-enterprise-applications.md)接口，已接入用户不受影响。
...
>
> - 如果未使用本接口，推荐使用新版规范[创建企业内部应用](0860-create-an-h5-application-for-your-enterprise.md)接口。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6d1bxxxx | 调用服务端接口的授权凭证，可通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| appIcon | String | 是 | @lADPDfmVQafoVxxxx | H5微应用的图标。  需要调用[上传媒体文件](0645-upload-media-files.md)将图标上传到钉钉服务器后获取到的mediaId。 |
