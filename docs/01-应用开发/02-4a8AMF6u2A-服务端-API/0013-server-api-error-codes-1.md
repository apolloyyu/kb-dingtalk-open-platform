...
> Path: 应用开发 / 服务端 API / API 调用指南 > 全局错误码
...
| 400040 | 回调不存在 | 回调不存在。检查是否注册回调事件。可通过[查询订阅事件](../04-LFcRvVD08N-事件订阅/0241-query-subscribed-events.md)接口获取。 |  |
| 400041 | 回调已经存在 | 回调已经存在。检查该回调事件是否已注册过。可通过[查询订阅事件](../04-LFcRvVD08N-事件订阅/0241-query-subscribed-events.md)接口获取。 |  |
