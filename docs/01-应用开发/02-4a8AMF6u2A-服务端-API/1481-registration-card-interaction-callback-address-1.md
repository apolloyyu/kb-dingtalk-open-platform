...
> - 未接入的开发者建议使用新版[注册卡片回调地址](0787-register-card-callback-address.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | be3xxxx | 调用该接口的应用凭证。   - 企业内部应用可通过[获取企业内部应用的access\_token](1447-obtain-orgapp-token.md)接口获取。 - 第三方企业应用可通过[服务商获取第三方应用授权企业的access\_token](1449-obtain-isvapp-token.md)接口获取。 |
...
| callbackRouteKey | String | 否 | xxxxxx | callback地址的路由Key，一个`callbackRouteKey`仅可映射一个`callback_url`。该参数在[发送钉钉互动卡片（高级版）](1479-send-interactive-dynamic-cards-1.md)接口中使用。 |
