...
> Path: 应用开发 / 服务端 API / 文档/文件 > 表格 > 单元格区域 > 更新单元格区域
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证，可通过以下方式获取：   - 企业内部应用，调用[获取企业内部应用的accessToken](0032-obtain-the-access-token-of-an-internal-app.md)接口获取。 - 第三方企业应用，调用[获取第三方应用授权企业的accessToken](0033-obtain-the-access-token-of-the-authorized-enterprise-1.md)接口获取。 |

### 路径参数

| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| workbookId | String | 是 | 表格文件 ID ，[知识库 API](0552-knowledge-base-overview.md) 返回的`nodeId(dentryUuid)`即是表格`workbookId`，可通过调用[获取节点](https://open.dingtalk.com/document/development/get-node)和[创建知识库文档](0566-create-team-space-document.md)接口获取。 |
| sheetId | String | 是 | 工作表ID或名称。   - 企业内部应用，可调用[获取所有工作表](0594-obtain-all-worksheets.md)接口获取id或name参数值。 - 第三方企业应用，可调用[创建工作表](0590-create-a-worksheet.md)接口获取id和name。 |
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| operatorId | String | 是 | 操作人的unionId，可调用[查询用户详情](0055-query-user-details.md)接口获取。      若操作人无权限，接口会报错 `The operator has no permission`。 |
