...
> Path: 应用开发 / 服务端 API / 钉钉应用 > 版本管理 > 回滚企业内部小程序版本
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| appVersionId | Long | 是 | 小程序版本ID，用于唯一标识一个历史版本。可通过调用[获取企业内部小程序历史版本列表](0869-obtain-the-list-of-historical-versions-of-enterprise-internal-applets.md)接口，在返回结果中获取`appVersionId`字段值。 |
| opUnionId | String | 是 | 操作人的unionId，必须为当前企业内拥有应用管理权限的有效管理员，可通过[查询用户详情](0055-query-user-details.md)接口获取`unionId`参数值。 |
