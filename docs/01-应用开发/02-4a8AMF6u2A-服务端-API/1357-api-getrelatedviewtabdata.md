...
| formCode | String | 否 | 表单代码，通过[获取审批中创建与CRM客户关联的TAB表单元数据](1356-api-getrelatedviewtabmeta.md)返回参数`formCode`获取。 |
| maxResults | Integer | 否 | 每页最大的个数 |
| relatedField | String | 否 | 关联字段，通过[获取审批中创建与CRM客户关联的TAB表单元数据](1356-api-getrelatedviewtabmeta.md)返回参数`relateComponentId`获取。 |
