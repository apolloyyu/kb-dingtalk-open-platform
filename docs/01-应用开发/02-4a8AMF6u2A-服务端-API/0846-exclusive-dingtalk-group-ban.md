...
> Path: 应用开发 / 服务端 API / 专属钉钉 > 企业内部群 > 企业内部群禁言或解除禁言
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| openConverationId | String | 是 | 群ID，获取方式如下   1. 拥有**专属安全-群管理**权限的管理员登录[钉钉管理后台](https://oa.dingtalk.com) > 专属钉钉 > 专属安全 > 群管理中读取，如图。 2. 通过接口获取，可调用[查询企业内部群信息](0845-obtain-group-info.md)接口获取。 |
