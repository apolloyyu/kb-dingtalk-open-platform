...
> Path: 应用开发 / 服务端 API / 认证与授权 > 访问凭证 > 用户个人身份凭证 > 获取用户token
...
| code | String | 否 | abcd | OAuth 2.0 临时授权码，第三方企业应用需要[接入统一授权套件](0010-unified-licensing-suite-sdk.md)/[获取登录用户的访问凭证](0028-obtain-identity-credentials.md)，获取临时授权码authCode。 |
