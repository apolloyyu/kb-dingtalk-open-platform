...
1. 调用服务端API-[创建部门](0076-address-book-creation-department-established-department.md)接口，根据企业自有OA系统内该部门层级和部门名称，在钉钉客户端内创建同样的部门层级和部门名称。
2. 调用服务端API-[创建角色组](0090-add-a-role-group.md)接口，创建角色组信息。
3. 调用服务端API-[创建角色](0085-address-book-add-role.md)接口，创建企业角色信息。
4. 调用[创建用户](0054-user-information-creation.md)接口，把企业OA系统内各个部门内的员工创建添加到钉钉对应的部门内，本步骤添加用户的同时，可以同时添加该员工的角色信息。
...
3. 根据获取的 `Client ID` 和 `Client Secret`，调用[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口，获取应用访问凭证。
...
4. 企业OA系统通讯录内创建了部门，调用[创建部门](0076-address-book-creation-department-established-department.md)接口，根据企业自有OA系统内该部门层级和部门名称，在钉钉客户端内创建同样的部门层级和部门名称。
...
5. 调用服务端API-[创建角色组](0090-add-a-role-group.md)接口，创建角色组信息。
   ...
   ```
6. 调用服务端API-[创建角色](0085-address-book-add-role.md)接口，创建企业角色信息。
   ...
   ```
7. 调用[创建用户](0054-user-information-creation.md)接口，把企业OA系统内各个部门内的员工创建添加到钉钉对应的部门内，本步骤添加用户的同时，可以同时添加该员工的角色信息。
