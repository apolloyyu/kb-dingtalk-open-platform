...
1. 调用服务端API-[创建客户群组](1384-crm-create-group.md)接口，进行创建客户群组，获取客户群组`openGroupSetId`。
2. 根据客户群组`openGroupSetId`进行客户群组管理操作。

   - 根据客户群组`openGroupSetId`，调用服务端API-[更新客户群组](1386-crm-update-group.md)接口，进行更新客户群组。
   - 根据客户群组`openGroupSetId`，调用服务端API-[获取单个客户群组详情](1385-queries-the-details-of-a-single-customer-group.md)接口，进行查询单个客户群组详情。
3. 调用服务端API-[查询客户群组列表](1387-query-groups.md)接口，进行查询客户群组列表。

客户群相关API：

1. 调用服务端API-[创建客户群](1380-create-a-customer-group.md)接口，进行创建客户群。
2. 根据客户群`openConversationId`进行客户群管理操作。

   - 根据客户群`openConversationId`，调用服务端API-[获取单个客户群详情](1382-obtain-a-single-customer-group.md)接口，进行获取单个客户群详情。
   - 根据客户群`openConversationId`，调用服务端API-[批量查询客户群](1383-query-customer-groups-in-batches.md)接口，进行批量批量查询客户群详情。
3. 调用服务端API-[查询客户群列表](1381-query-the-list-of-customer-groups.md)接口，进行查询客户群列表。
...
1. 调用服务端API-[创建客户群组](1384-crm-create-group.md)接口，进行创建客户群组，获取客户群组openGroupSetId。
...
2. 根据客户群组`openGroupSetId`进行客户群组管理操作。
   ...
   - 根据客户群组openGroupSetId，调用服务端API-[更新客户群组](1386-crm-update-group.md)接口，进行更新客户群组。
...
2. 根据客户群`openConversationId`进行客户群管理操作。
   ...
   - 根据客户群openConversationId，调用服务端API-[获取单个客户群详情](1382-obtain-a-single-customer-group.md)接口，进行获取单个客户群详情。
