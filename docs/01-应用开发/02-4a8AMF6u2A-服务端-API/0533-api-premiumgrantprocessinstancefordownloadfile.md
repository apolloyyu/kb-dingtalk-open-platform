...
> Path: 应用开发 / 服务端 API / OA 审批 > 高级版专享接口 > 官方 OA 审批 > 审批钉盘空间&附件 > 下载审批附件
...
| fileId | String | 是 | 文件fileId，可调用[获取单个审批实例详情](0497-obtains-the-details-of-a-single-approval-instance-pop.md)接口获取      文件id是审批组件中上传的fileId（如下图所示），评论中上传的附件fileId也支持获取下载链接。 |
