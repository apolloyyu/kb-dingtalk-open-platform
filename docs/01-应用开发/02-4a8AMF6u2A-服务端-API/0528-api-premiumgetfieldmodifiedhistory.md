...
> Path: 应用开发 / 服务端 API / OA 审批 > 高级版专享接口 > 官方 OA 审批 > 审批实例 > 获取审批表单控件字段内容修改记录
...
| fieldId | String | 是 | 审批表单控件ID：   - 企业内部应用，与[创建或更新审批表单模板](0490-create-an-approval-form-template.md)接口中组件`componentId`字段值保持一致。 |
