...
> Path: 应用开发 / 服务端 API / 即时通信 > 会话管理 > 场景群 > 群管理 > 查询场景群内群模板机器人
...
支持以下场景使用：基于群模板创建的群，详情参见[创建场景群](0747-create-a-scene-group.md)。
...
| openConversationId | String | 否 | 群ID，可调用[创建场景群](0747-create-a-scene-group.md)接口获取`open_conversation_id`参数值。 |
