...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| class\_id | Number | 是 | 4240018 | 班级ID，可调用[获取部门列表](1153-obtains-the-department-node-list.md)接口获取dept\_type为class时的dept\_id参数值。 |
| name | String | 是 | 阳xx | 学生姓名。 |
| biz\_id | String | 否 | biz\_id | 业务的唯一ID，自定义值，每次调用保持唯一。 |
| student\_no | String | 否 | no12 | 学生学号，可调用[获取人员列表](1154-obtains-a-list-of-home-school-user-identities.md)接口获取student\_no参数值。 |
