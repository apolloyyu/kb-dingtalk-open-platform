...
> Path: 应用开发 / 服务端 API / 认证与授权 > 访问凭证 > 获取登录用户的访问凭证
...
使用返回的auth\_code和应用的信息，调用[获取用户token](0031-obtain-user-token.md)接口得到access\_token。
...
获取access\_token后，就可以使用这个凭证调用[获取用户通讯录个人信息](0053-dingtalk-retrieve-user-information.md)接口。
