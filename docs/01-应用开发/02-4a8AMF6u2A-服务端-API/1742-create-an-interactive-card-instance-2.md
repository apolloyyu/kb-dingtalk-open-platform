...
> - 未接入的开发者建议使用新版[创建并开启互动卡片吊顶](0762-send-group-helper-message.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| cardTemplateId | String | 是 | 卡片模板ID，可通过[管理消息模板](0766-manage-message-templates.md)获得。 |
| openConversationId | String | 否 | 接收卡片的群的openConversationId，可调用[创建群](1487-create-a-scene-group-v2.md)接口获取。 |
...
| pullStrategy | Boolean | 否 | 是否开启卡片纯拉模式：（默认不开启卡片纯拉模式）   - **true**：开启卡片纯拉模式 - **false**：不开启卡片纯拉模式   **[!NOTE]**  纯拉模式，请参考[实现置顶卡片纯拉模式](0737-pure-pull-mode-process-guide.md)。 |
