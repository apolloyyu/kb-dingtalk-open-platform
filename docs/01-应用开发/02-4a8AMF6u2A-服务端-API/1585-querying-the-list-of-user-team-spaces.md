...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 文档/文件 > 知识库 > 知识库管理 > 查询用户有权限的知识库列表
...
> - 未接入的开发者建议使用新版[获取知识库列表](0559-get-knowledge-base-list.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| operatorId | String | 是 | 被获取信息的用户unionId，调用[查询用户详情](0055-query-user-details.md)接口获取unionid参数值。  例如：需要查询小钉拥有权限的知识库列表，该参数传小钉的unionId。 |
