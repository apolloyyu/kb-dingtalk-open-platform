...
> Path: 应用开发 / 服务端 API / 更多开放 > 客户管理（官方CRM） > 客户群 > 批量查询客户群
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| openConversationIds | Array of String | 否 | 客户群openConversationId，调用[查询客户群列表](1381-query-the-list-of-customer-groups.md)接口获取openConversationId参数值。 |
...
| openGroupSetId | String | 客户群所在的群组openGroupSetId。   - 如果是客户群组裂变出的客户群，会返回该字段。 - 如果是调用[创建客户群](1380-create-a-customer-group.md)接口创建的客户群，不返回该字段。 |
