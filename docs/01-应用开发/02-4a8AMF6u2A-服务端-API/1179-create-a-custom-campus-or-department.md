...
> Path: 应用开发 / 服务端 API / 行业与生态 > 行业开放 > 新教育 > 家校通讯录2.0 > 创建自定义校区或部门
...
| superId | Long | 是 | 上级部门ID。   - 当**type**取值为**custom\_campus**时，必须为-**7**。 - 当**type**取值为**custom\_dept**时：调用[获取部门列表](1157-obtains-the-department-node-list.md)接口获取dept\_id参数值。 |
