...
- **发出来的卡片不符合预期**
  ...
  使用[卡片调试工具](0780-card-debugging-tool.md)查看下发的卡片数据是否正确。
- **设置卡片数据中非String类型的参数如何处理**

  - **新版接口**：如果是新版接口，包括[创建卡片](0781-interface-for-creating-a-card-instance.md)、[创建并投放卡片](0784-create-and-deliver-cards.md)和[更新卡片](0783-interactive-card-update-interface.md)，按照[API 卡片数据的填写说明](0790-instructions-for-filling-in-api-card-data.md)填写卡片数据即可。
  - **旧版接口**：例如机器人[发送钉钉互动卡片（高级版）](1479-send-interactive-dynamic-cards-1.md)，需要使用`sys_full_json_obj` 字段，比如创建、更新卡片的接口是用 `Map<String, String>` 传递卡片数据，这种方式参数值只能传递 String 类型。`sys_full_json_obj` 字段是卡片数据中内置的一个参数，参数值格式是`JSONString`，如需设置卡片的非String参数值，参考以下步骤：
...
- **如何关闭吊顶卡片？**
  ...
  2. **第二步：通过调用服务端API-**[关闭互动卡片吊顶](0763-close-interactive-card-ceiling.md)**实现关闭。**

     > **[!IMPORTANT]**
     >
     > 设置回传请求需调用[注册卡片回调地址](0787-register-card-callback-address.md)。
