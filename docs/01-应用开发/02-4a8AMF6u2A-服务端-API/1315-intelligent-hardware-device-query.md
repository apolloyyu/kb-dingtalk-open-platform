...
| device\_name | String | 否 | 产品智能 | 设备名称，可通过[查询设备列表](1314-intelligent-hardware-list-query.md)接口获取。  **[!NOTE]**  该参数不能和`device_id`同时为空。 |
| device\_id | String | 否 | QWR45GT | 设备id，可通过[查询设备列表](1314-intelligent-hardware-list-query.md)接口获取。  **[!NOTE]**  该参数不能和`device_name`同时为空。 |
