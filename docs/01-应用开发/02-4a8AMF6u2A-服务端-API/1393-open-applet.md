...
| target | 否 | 打开的容器类型：   - 移动端：    - **fullScreen**（默认）：全屏容器打开   - **panel**：沉浸式容器打开  **[!NOTE]**  什么是沉浸式容器，请参考[沉浸式容器](1424-immersive-container-1.md)。 - 桌面端：    - **slide**（默认），侧边栏打开   - **panel**：第四栏打开   - **popupWindow**：独立弹窗打开 |
