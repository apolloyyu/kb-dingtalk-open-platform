...
> Path: 应用开发 / 服务端 API / 平台公告与计费 > 资源与计费 > API调用量配额 > 调用频次与限流
...
> **[!NOTE]**
>
> 标准版钉钉组织，该组织内所有企业内部应用、委托第三方定制开发企业内部应用，除不纳入每月调用量限制的接口（包括获取访问凭证接口、身份验证接口、通讯录接口等，详细接口请查看[不纳入调用量限制的接口清单](1433-basic-interfaces-such-as-log-off-and-address-book.md)）之外，其他接口累计可调用次数调整为5000次/月。
