...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 获取访问凭证 > 获取jsapi_ticket
...
> - 未接入的开发者建议使用新版 [获取jsapiTicket](0038-create-a-jsapi-ticket.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6adfxxxx | 调用该接口的应用凭证。   - 企业内部应用，通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 - 第三方企业应用，通过[服务商获取第三方应用授权企业的access\_token](1445-obtain-isvapp-token.md)接口获取。 |
