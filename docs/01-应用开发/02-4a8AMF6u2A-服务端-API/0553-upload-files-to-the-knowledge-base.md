...
> Path: 应用开发 / 服务端 API / 文档/文件 > 知识库 > 使用教程 > 知识库上传文件
...
- `parentDentryUuid`（Path）：上传目标目录的 dentryUuid。若上传到知识库根目录，可通过以下两种方式获取根目录的dentryUuid：
  ...
  - 调用[获取我的文档知识库信息](0562-get-my-documents.md)接口，取返回结果`workspace`下的`rootNodeId`字段值；
  - 调用[获取知识库列表](0559-get-knowledge-base-list.md)接口，取返回结果`workspaces`下的`rootNodeId`字段值。
