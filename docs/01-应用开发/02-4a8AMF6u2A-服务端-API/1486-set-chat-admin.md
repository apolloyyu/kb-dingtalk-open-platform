...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 即时通信 > 会话管理 > 群管理 > 更新群管理员
...
> - 未接入的开发者建议使用新版[批量设置企业群管理员](0743-batch-setup-group-administrator.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| chatid | String | 是 | chat14432xxxx | 群会话ID，可通过[创建群](1484-session-management-creates-groups.md)接口获取chatid参数值。 |
