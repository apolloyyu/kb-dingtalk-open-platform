...
> **[!IMPORTANT]**
>
> - 企业内部应用推荐直接通过开发者后台订阅回调事件，详情请参考[配置事件推送方式](0003-configure-stream-push.md)。
> - 第三方企业应用推荐使用SyncHTTP推送和RDS推送方式订阅回调事件，这两种方式可直接使用推送的数据，配置更简单。
> - HTTP推送的出口IP为203.119.0.0/16 140.205.0.0/16 106.11.0.0/16 198.11.0.0/16。

## 使用场景

- 在你的业务对数据的实时性要求较高时。例如：在新员工入职或者离职时，应用需要第一时间变更用户数据，此时就可以订阅[通讯录事件](0343-address-book-events.md)。
- 管理员开通第三方企业应用时，向应用推送[企业授权开通应用事件](0248-enterprise-authorization-to-open-application-event-history.md)，开发者可以根据推送的corpid进行企业激活和应用开通。
...
4. 调用[注册回调事件](../02-4a8AMF6u2A-服务端-API/1861-register-callback-events.md)接口接收回调事件。
...
3. 配置用于接收请求的HTTP地址。
   ...
   - **数据加密密钥**：单击**自动生成**生成AES密钥。回调消息内容的加解密参数，是AES密钥的Base64编码。
