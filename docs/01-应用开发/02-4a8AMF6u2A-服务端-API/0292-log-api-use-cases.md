...
> Path: 应用开发 / 服务端 API / 日志 > 使用教程 > 三方系统发起和查看日志信息
...
3. 调用[保存日志内容](0294-save-custom-log-content.md)接口，获取contentId。
4. 拼接URL，在三方系统页面内访问该URL，可以跳转进入日志发起页面，拼接的URL如下
   ...
   1. **URL参数说明**
      ...
      - app\_id：固定值2。
      - templateid：日志模板Id，调用[获取模板详情](0295-query-template-details.md)接口获取。
      - contentid：调用[保存日志内容](0294-save-custom-log-content.md)接口返回的contentId。
