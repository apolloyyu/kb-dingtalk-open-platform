...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| userId | String | 否 | 关注服务窗用户的userId，可调用[批量获取关注服务窗用户信息](1285-obtains-the-follower-information-from-the-service-window.md)获得userId参数值。  **[!NOTE]**     - 服务窗应用场景下，该参数必填，不能使用unionId。 - 非服务窗应用场景下，该参数和unionId任选其一。 |
...
| accountId | String | 否 | 服务窗账号ID，通过[获取企业下服务窗列表](1283-queries-the-list-of-services-under-an-enterprise.md)接口获得。  **[!NOTE]**     - 服务窗应用场景下，该参数不传，否则会报错非法请求。 - 非服务窗应用场景下，该参数必填，否则会报错非法服务窗账号。 |
