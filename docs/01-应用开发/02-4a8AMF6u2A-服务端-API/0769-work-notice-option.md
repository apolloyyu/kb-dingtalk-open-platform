...
> Path: 应用开发 / 服务端 API / 即时通信 > 消息通知 > 使用教程 > 发送、查询、撤回工作通知
...
1. 调用服务端API-[发送工作通知](0770-asynchronous-sending-of-enterprise-session-messages.md)接口，发送工作通知，获取工作通知消息唯一任务ID`task_id`。
2. 根据唯一任务ID`task_id`，调用服务端API-[获取工作通知消息的发送进度](0773-obtain-the-sending-progress-of-asynchronous-sending-of-enterprise-session.md)接口，获取发送进度。
3. 根据唯一任务ID`task_id`，调用服务端API-[获取工作通知消息的发送结果](0774-gets-the-result-of-sending-messages-asynchronously-to-the-enterprise.md)接口，查看发送结果。
4. 根据唯一任务ID`task_id`，调用服务端API-[更新工作通知状态栏](0772-update-work-notification-status-bar.md)接口，更新工作通知。
5. 根据唯一任务ID`task_id`，调用服务端API-[撤回工作通知消息](0771-notification-of-work-withdrawal.md)接口，撤回工作通知。
...
根据步骤一中的 Client ID 和 Client Secret，获取应用访问凭证[获取企业内部应用的access\_token](1447-obtain-orgapp-token.md)。
...
1. 调用服务端API-[发送工作通知](0770-asynchronous-sending-of-enterprise-session-messages.md)接口，发送工作通知，获取工作通知消息唯一任务ID task\_id。
   ...
   ```
2. 根据唯一任务ID task\_id，调用服务端API-[获取工作通知消息的发送进度](0773-obtain-the-sending-progress-of-asynchronous-sending-of-enterprise-session.md)接口，获取发送进度。
   ...
   ```
3. 根据唯一任务ID task\_id，调用服务端API-[获取工作通知消息的发送结果](0774-gets-the-result-of-sending-messages-asynchronously-to-the-enterprise.md)接口，查看发送结果。
   ...
   ```
4. 根据唯一任务ID task\_id，调用服务端API-[更新工作通知状态栏](0772-update-work-notification-status-bar.md)接口，更新工作通知。
   ...
   ```
5. 根据唯一任务ID task\_id，调用服务端API-[撤回工作通知消息](0771-notification-of-work-withdrawal.md)接口，撤回工作通知。
