...
> Path: 应用开发 / 服务端 API / 考勤 > 考勤班次 > 创建班次
...
| id | Number | 否 | 124 | 班次id，可通过[获取班次摘要信息](0202-enterprise-shift-query-in-batches.md)接口获取id参数值。  **[!NOTE]**  id 不传值时为创建班次，传值时是修改已有班次。 |
...
| class\_id | Number | 否 | 221 | 班次id，可通过[获取班次摘要信息](0202-enterprise-shift-query-in-batches.md)接口获取id参数值。 |
