...
> - 未接入的开发者建议使用新版 [更新用户信息](0056-user-information-update.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6d1bxxxx | 调用服务端API授权凭证，通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| userid | String | 是 | 1001 | 员工userid，不可修改，长度为1~64个字符，可通过[根据手机号查询用户](1462-retrieve-userid-from-mobile-phone-number.md)接口获取。 |
