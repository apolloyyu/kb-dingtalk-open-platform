...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 文档/文件 > 存储管理 > 权限管理 > 获取权限列表
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| spaceId | String | 是 | 空间Id，调用[添加空间](0651-add-space.md)接口获取id参数值。  **[!NOTE]**  如果空间类型为APP时，当前操作人只能查看本人在该空间内的权限，不能查看全部的权限。 |
| dentryId | String | 是 | 文件或文件夹Id，调用[获取文件或文件夹列表](0665-get-a-list-of-files-or-folders.md)接口获取id参数值。  **[!NOTE]**  该参数值传0时，获取的是空间根目录的权限列表信息。 |
