...
> - 未接入的开发者建议使用新版[根据指定条件查询自定义对象数据](1378-api-getobjectdata.md)接口，已接入用户不受影响。
