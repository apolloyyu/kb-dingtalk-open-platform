...
> Path: 应用开发 / 服务端 API / OA 审批 > 官方 OA 审批 > 审批实例 > 获取审批实例ID列表
...
2. 调用[获取单个审批实例详情](0497-obtains-the-details-of-a-single-approval-instance-pop.md)接口，获取审批实例详情信息。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| processCode | String | 是 | 审批流的唯一码。   - 调用[创建或更新审批表单模板](0490-create-an-approval-form-template.md)接口获取`processCode`参数值。 - 通过[名词解释-processCode](0472-workflow-overview.md)获取。 |
...
| userIds | Array of String | 否 | 发起人userId列表，最大列表长度为10，可通过[获取部门用户userid列表](0064-query-the-list-of-department-userids.md)接口获取。 |
