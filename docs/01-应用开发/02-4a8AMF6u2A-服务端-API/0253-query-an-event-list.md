...
> Path: 应用开发 / 服务端 API / 日程 > 日程 > 查询日程列表
> Updated: 2026-06-02 09:25:01
...
- 默认不会返回已删除的日程，如果需要返回可以设置`showDeleted=true`，当返回的是已删除的日程时，此时调用[查询单个日程详情](0252-query-details-about-an-event.md)接口会返回**instanceNotExist**的错误。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| userId | String | 是 | 查询目标用户的unionId。   - 企业内部应用和第三方企业应用，调用[查询用户详情](0055-query-user-details.md)接口获取unionid参数值。 - 第三方个人应用，调用[获取用户通讯录个人信息](0053-dingtalk-retrieve-user-information.md)接口获取unionId参数值。 |
