...
> - 未接入的开发者建议使用新版[提交文件](0676-submittal-file.md)接口，已接入用户不受影响。

### 接口调用步骤

步骤一：调用[获取文件上传信息](1603-obtain-storage-upload-information.md)接口，获取上传文件需要的信息。
...
>
> 可调用[添加权限](1599-add-storage-permissions.md)接口进行授权。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| unionId | String | 是 | 操作者unionId，调用[查询用户详情](0056-query-user-details.md)接口获取。 |

## Body参数

| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| uploadKey | String | 是 | 添加文件唯一标识，调用[获取文件上传信息](1603-obtain-storage-upload-information.md)接口获取uploadKey参数值。 |
| name | String | 是 | 文件的名称，带后缀。命名有以下要求：   - 头尾不能包含空格，否则会自动去除 - 不能包含特殊字符，包括：制表符、\*、"、<、>、| - 不能以"."结尾 |
| parentId | String | 是 | 父目录Id。根目录时，该参数是0，可调用[获取文件或文件夹列表](0666-get-a-list-of-files-or-folders.md)接口获取parentId参数值。 |
