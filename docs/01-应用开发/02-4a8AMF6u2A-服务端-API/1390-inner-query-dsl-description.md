...
| 类别 | 接口列表 |
| --- | --- |
| 客户 | - [根据指定条件查询个人或企业客户数据](1352-obtains-crm-individual-customers-in-batches-based-on-specified-query.md) |
| 联系人 | - [根据指定条件查询联系人数据](1362-api-getcontacts.md) |
| 跟进记录 | - [根据指定条件查询跟进记录数据](1368-query-and-dingtalk-data-of-track-records-in-apsara-stack.md) |
| 自定义对象 | - [根据指定条件查询自定义对象数据](1374-api-getobjectdata.md) |
...
| fieldId | **客户：**  过滤字段名，支持传入两种字段类型：   - **表单字段**：通过调用[获取个人或企业客户的元数据](1344-obtain-the-metadata-of-individual-enterprise-customers.md)接口，获取字段列表fields下的字段名称name - **系统字段**：    - gmt\_create：创建时间   - gmt\_modified：更新时间   - principal：负责人   - participant：协同人 |
| **联系人：**  过滤字段名，支持传入两种字段类型：   - **表单字段**：通过调用[获取联系人的元数据](1361-gets-the-metadata-description-of-a-crm-contact-object.md)接口，获取字段列表fields下的字段名称name。 - **系统字段**：    - gmt\_create：创建时间   - gmt\_modified：更新时间   - principal：负责人   - participant：协同人 |
