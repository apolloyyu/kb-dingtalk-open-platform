...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| taskId | Long | 是 | 审批任务ID，可调用[获取单个审批实例详情](0497-obtains-the-details-of-a-single-approval-instance-pop.md)接口获取。 |
...
| targetActivityId | String | 是 | 退回到的节点ID。   - 可调用[获取审批单流程中的节点信息](0492-approval-process-prediction.md)接口获取审批单流程中的节点ID信息`activityId`参数值。 - 若退回方式为`REVERT_FOR_RESUBMIT` 退回到发起人，则targetActivityId固定传发起节点的ID值：`sid-startevent` |
