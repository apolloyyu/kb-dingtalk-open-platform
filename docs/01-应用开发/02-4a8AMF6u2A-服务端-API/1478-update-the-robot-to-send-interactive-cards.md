...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 即时通信 > 机器人 > 更新机器人发送互动卡片（普通版）
...
> - 未接入的开发者建议使用新版[更新卡片](0783-interactive-card-update-interface.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| cardBizId | String | 是 | 唯一标识一张卡片的外部ID（卡片幂等ID，可用于更新或重复发送同一卡片到多个群会话）。需与机器人发送互动卡片（普通版）接口Body参数中cardBizId保持一致，请参见[机器人发送互动卡片（普通版）](1477-robots-send-interactive-cards.md)接口。 |
...
| 名称 | 类型 | 描述 |
| --- | --- | --- |
| processQueryKey | String | 加密消息id，根据此id可查询消息已读状态和撤回消息，请参考[查询企业机器人群聊消息用户已读状态](0723-chatbot-queries-the-read-status-of-a-message.md)和[企业机器人撤回内部群消息](0726-enterprise-chatbot-withdraws-internal-group-messages.md)。 |
