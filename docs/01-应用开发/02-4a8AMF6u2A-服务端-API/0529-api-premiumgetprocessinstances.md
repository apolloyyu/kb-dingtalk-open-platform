...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证，企业内部应用可调用[获取企业内部应用的accessToken](0032-obtain-the-access-token-of-an-internal-app.md)接口获取。 |
...
| processCode | String | 是 | 模板ID。   - 通过调用[创建或更新审批表单模板](0490-create-an-approval-form-template.md)接口获取。 - 通过[OA审批概述-名词解释](0472-workflow-overview.md)获取。 |
