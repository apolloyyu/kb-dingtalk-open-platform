...
> Path: 应用开发 / 服务端 API / 行业与生态 > 生态开放 > AI > 使用教程 > 实现文本内容翻译
...
- 调用服务端API-[钉钉文本翻译](1010-dingtalk-translation.md)接口后，实现文本内容翻译。
...
步骤四：调用[获取企业内部应用的access\_token](1447-obtain-orgapp-token.md)接口，获取应用访问凭证access\_token（有效期2小时，建议服务端缓存并自动刷新。

步骤五：调用服务端API-[钉钉文本翻译](1010-dingtalk-translation.md)接口，实现文本内容翻译。
...
根据步骤二中的AppKey和AppSecret，调用[获取企业内部应用的access\_token](1447-obtain-orgapp-token.md)接口获取组织级access\_token。

## 步骤五：调用服务端AI相关API

调用服务端API-[钉钉文本翻译](1010-dingtalk-translation.md)接口，实现文本内容的翻译。
