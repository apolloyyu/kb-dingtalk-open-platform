...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6d1bxxxx | 调用服务端API的应用凭证，可通过[获取企业内部应用的access\_token](1447-obtain-orgapp-token.md)接口获取。 |
...
>
> 1. 调用[获取审批钉盘空间信息](1537-query-the-space-of-an-approval-nail.md)，获取钉盘空间的上传权限，并获取space\_id。
...
| process\_instance\_id | String | 是 | 88c2f806-xxxx | 审批实例ID，调用[获取审批实例ID列表](1534-operation-to-retrieve-a-list-of.md)接口获取。 |
| file | File | 否 |  | 文件。 |
| attachments | Attachment[] | 否 |  | 附件列表。 |
| space\_id | String | 否 | 232323 | 钉盘空间ID，调用[获取审批钉盘空间信息](1537-query-the-space-of-an-approval-nail.md)接口获取。 |
