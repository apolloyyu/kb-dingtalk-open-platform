...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > OA审批 > 批量取消待办
...
> - 未接入的开发者建议使用新版[批量取消流程中心待处理任务](0518-cancel-multiple-oa-approval-tasks.md)接口，已接入用户不受影响。
...
| process\_instance\_id | String | 是 | EF6YJL35 | 实例ID，由[创建实例](1514-initiate-an-approval-process-without-a-process.md)接口获取。 |
| activity\_id | String | 是 | 1111 | 待办组ID，需要在调用[查询待办列表](1521-query-a-user-s-to-do-items.md)接口时，主动设置该值。 |
...
| 8100017 | 无操作审批流的权限，请检查审批实例或者模板是否正确 | 无操作审批流的权限，审批实例或者模板参数错误 | 实例ID（process\_instance\_id）必须是[创建实例](1514-initiate-an-approval-process-without-a-process.md)接口返回的process\_instance\_id值，不能使用官方审批流的实例值。 |
