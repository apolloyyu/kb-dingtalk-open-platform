...
> Path: 应用开发 / 服务端 API / 应用授权 > 获取授权应用的基本信息
...
- 在使用HTTP调用本接口时，必须设置**signature**参数，钉钉会对请求进行签名验证，用以提升安全水位。签名计算方式，请参考[第三方访问接口的签名计算方法](1426-the-signature-calculation-method-of-the-third-party-access-interface.md)。
...
| signature | String | 是 | xxxx | 签名。签名计算方式请参考[第三方访问接口的签名计算方法](1426-the-signature-calculation-method-of-the-third-party-access-interface.md)。  **[!IMPORTANT]**  计算出签名以后，需要进行urlencode，才能把签名参数拼接到url中。 |

### **请求体**

| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| agentid | String | 是 | 852381775 | 授权企业方应用ID，有以下两种获取方式：   - 调用[获取企业授权信息](0041-obtains-the-basic-information-of-an-enterprise.md)接口获取。 - 从[企业授权开通应用事件](../04-LFcRvVD08N-事件订阅/0005-development-data-format-help.md)中获取。 |
