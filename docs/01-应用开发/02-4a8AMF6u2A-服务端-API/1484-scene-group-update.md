...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| open\_conversation\_id | String | 是 | cidt\*\*\*\*\*Xa4K10w== | 群ID，调用[创建群](1483-create-a-scene-group-v2.md)接口获取`open_conversation_id`参数值。 |
| title | String | 否 | 测试群 | 群名称。  **[!NOTE]**  最长不超过30字符，建议长度在10字符以内。 |
| owner\_user\_id | String | 否 | 072\*\*\*\*\* | 群主的userId。 |
| icon | String | 否 | @lADOADma\*\*\*\*\*QKA | 群头像，格式为`mediaId`，调用[上传媒体文件](0645-upload-media-files.md)接口获取mediaId参数值。 |
