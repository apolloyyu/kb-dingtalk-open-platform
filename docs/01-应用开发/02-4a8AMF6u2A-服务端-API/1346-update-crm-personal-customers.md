...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| instanceId | String | 是 | 客户数据ID，调用[根据指定条件查询个人或企业客户数据](1352-obtains-crm-individual-customers-in-batches-based-on-specified-query.md)接口获取instanceId参数值。 |
| modifierUserId | String | 是 | 操作人的用户userId。 |
| modifierNick | String | 否 | 操作人的用户昵称。 |
| data | Map | 是 | 客户数据内容，JSON格式字符串，格式请参见[新增和更新客户字段格式说明V1](1386-add-and-update-customer-field-format-description-v1.md)。 |
