...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 智能人事 > 员工管理 > 获取员工离职信息
...
> - 未接入的开发者建议使用新版[批量获取员工离职信息](0950-obtain-resignation-information-of-employees-new-version.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| userid\_list | String | 是 | user123,user456 | 要查询的离职员工userid，多个员工用逗号分隔，最大长度50，通过[获取离职员工列表](0948-obtain-the-list-of-employees-who-have-left.md)接口获取data\_list参数值。  如果传入为非离职员工userid，不会返回信息。 |
