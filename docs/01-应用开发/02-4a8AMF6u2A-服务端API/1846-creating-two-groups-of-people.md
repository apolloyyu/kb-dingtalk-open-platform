...
| users | Array | 否 | 群成员列表。 |
| appUserId | String | 否 | 钉外账号在业务系统内的唯一标志，调用[创建钉钉客联钉外账号](1844-create-bc-account-association.md)接口获取，通过给该字段赋值来标识当前群成员为钉外账号，长度限制为1～64个字符。例如：1107\*\*\*\*2120。  **[!NOTE]**    与userId字段二选一填值，不可都传或都不传。 |
