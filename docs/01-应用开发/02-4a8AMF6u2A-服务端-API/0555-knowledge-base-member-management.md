...
> Path: 应用开发 / 服务端 API / 文档/文件 > 知识库 > 使用教程 > 知识库成员管理
...
1. 调用[获取知识库列表](0559-get-knowledge-base-list.md)接口，从返回结果中获取目标知识库的`rootNodeId`字段，即知识库根节点的`dentryUuid`。
2. 获取成员 ID：

   - 添加**用户**：调用[查询用户详情](0055-query-user-details.md)接口获取`userId`，或通过[获取部门用户userid列表](0064-query-the-list-of-department-userids.md)批量获取；
   - 添加**部门**：调用[获取部门列表](0081-user-management-acquires-the-list-departments.md)接口获取`deptId`；
   - 添加**群会话**：使用群的`chatId`。
