...
> Path: 应用开发 / 服务端 API / 文档/文件 > 知识库 > 使用教程 > 知识库权限变更
...
- `dentryUuid`（Path）：目标文件/文件夹的唯一标识，通过[搜索文件](0637-search-for-files.md)接口获取；
- `unionId`（Query）：操作者的 unionId，可调用[查询用户详情](0055-query-user-details.md)接口获取；
...
1. 通过[搜索文件](0637-search-for-files.md)接口，获取目标文件的 `dentryUuid`；
2. 通过[查询用户详情](0055-query-user-details.md)接口，获取目标用户的`userId`和操作者的`unionId`
3. 调用[添加权限](0681-add-permissions-file.md)接口，`roleId = "EDITOR"`，`members[0].type = "USER"`，`members[0].id = 目标用户 userId`。

### 场景二：将某用户的权限从编辑者降级为只读

1. 通过[搜索文件](0637-search-for-files.md)接口，获取目标文件的 `dentryUuid`；
2. 通过[查询用户详情](0055-query-user-details.md)接口，获取目标用户的`userId`；
...
1. 通过[搜索文件](0637-search-for-files.md)接口，获取子文件夹的 `dentryUuid`；
