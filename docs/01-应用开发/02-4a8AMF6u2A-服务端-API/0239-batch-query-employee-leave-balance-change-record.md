...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| opUserId | String | 是 | 当前企业内拥有OA审批应用权限的管理员的userId，建议传入企业主管理员userId，可调用[获取管理员列表](0067-query-the-administrator-list.md)接口，获取返回参数主管理员`userId`字段。 |
| leaveCode | String | 是 | 假期类型唯一标识，调用[查询假期规则列表](0237-holiday-type-query.md)接口获取`leave_code`参数值。 |
