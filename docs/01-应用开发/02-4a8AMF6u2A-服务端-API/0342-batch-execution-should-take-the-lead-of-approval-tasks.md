...
> Path: 应用开发 / 服务端 API / 宜搭 > 任务 > 批量执行宜搭审批任务
> Updated: 2026-06-03 10:11:56
...
| taskInformationList | String | 是 | 批量执行的审批任务列表，数组对象格式，每个元素值包含taskId和formInstId两个子属性。       - taskId，表示待执行的审批任务Id，调用[查询流程运行任务（VPC）](0345-query-process-running-tasks-vpc.md)接口获取。 - formInstId，表示当前taskId所属的宜搭表单实例Id，调用[获取实例ID列表](0309-api-getinstanceidlist-v2.md)接口或者[获取多个表单实例ID](0323-api-searchformdataidlist-v2.md)接口获取。   该参数示例值如下：   ``` "[{\"taskId\":\"2291xxx\",\"formInstId\":\"d84a79xxx\"}, {\"taskId\":\"2291xxx\",\"formInstId\":\"f8035e2axxx\"}]" ``` |
