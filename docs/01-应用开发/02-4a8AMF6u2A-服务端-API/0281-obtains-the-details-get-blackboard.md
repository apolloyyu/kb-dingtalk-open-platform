...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证，企业内部应用通过调用[获取企业内部应用的accessToken](0032-obtain-the-access-token-of-an-internal-app.md)接口获取。 |
...
| blackboardId | String | 否 | 公告id，可以通过[获取公告ID列表接口](0282-obtains-the-id-list-of-announcements-that-are-not-deleted.md)获取id参数值。 |
