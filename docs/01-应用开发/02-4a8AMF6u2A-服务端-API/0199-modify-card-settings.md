...
> Path: 应用开发 / 服务端 API / 考勤 > 考勤班次 > 修改打卡时段设置
...
| id | Number | 是 | 123 | 卡点ID， 可通过[获取班次详情](0203-shift-query.md)接口获取id参数值。 |
| free\_check | Boolean | 是 | true | 是否无需打卡。   - **true**：开启无需打卡。 - **false**：关闭无需打卡。 |
| shift\_id | Number | 是 | 456 | 班次ID， 可通过[获取班次摘要信息](0202-enterprise-shift-query-in-batches.md)接口获取id参数值。 |
