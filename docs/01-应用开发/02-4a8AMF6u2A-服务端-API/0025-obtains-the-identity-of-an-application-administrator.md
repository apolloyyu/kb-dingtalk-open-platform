...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | tokenxxxx | 调用该接口的访问凭证，这里需要使用微应用后台免登的access\_token，可以通过调用[获取微应用后台免登的accessToken](0024-obtain-the-access-token-of-the-micro-application-background-without-log-on.md)接口获取。 |
