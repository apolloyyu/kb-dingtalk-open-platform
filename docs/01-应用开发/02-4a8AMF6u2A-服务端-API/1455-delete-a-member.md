...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 通讯录管理 > 用户管理1.0(不推荐) > 删除用户
...
> - 未接入的开发者建议使用新版 [删除用户](0057-delete-a-user.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6ed1bxxx | 调用服务端API授权凭证，通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 |
| userid | String | 是 | user123 | 员工唯一标识userid，可通过[根据手机号查询用户](1462-retrieve-userid-from-mobile-phone-number.md)接口获取userid。 |
