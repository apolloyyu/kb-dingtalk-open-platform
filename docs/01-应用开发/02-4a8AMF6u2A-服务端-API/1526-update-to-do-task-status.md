...
| process\_instance\_id | String | 是 | proc-zzz | 实例ID，由[创建实例](1518-initiate-an-approval-process-without-a-process.md)接口获取。 |
| tasks | TaskTopVo[] | 是 |  | 待办任务列表。 |
| task\_id | Number | 是 | 111 | 待办任务ID，需要在调用[查询待办列表](1525-query-a-user-s-to-do-items.md)接口时，主动设置该值。 |
