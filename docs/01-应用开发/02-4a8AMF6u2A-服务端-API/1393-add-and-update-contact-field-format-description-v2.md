...
> Path: 应用开发 / 服务端 API / 更多开放 > 客户管理（官方CRM） > 控件格式 > 新增和更新联系人字段格式说明V2
...
- **自定义控件字段**：自定义控件字段是用户可自定义的，用户可以随时进入客户管理后台新增或删除自定义控件字段，自定义控件字段的key可以通过调用[批量修改联系人数据](1364-modify-contact-data-in-batches.md)接口，获取字段列表fields下的字段名称name，自定义控件字段的传参格式详见[自定义控件字段格式说明V2](1389-custom-control-field-format-description-v2.md)。
