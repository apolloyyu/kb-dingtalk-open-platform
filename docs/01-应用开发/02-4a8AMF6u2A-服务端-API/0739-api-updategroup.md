...
> Path: 应用开发 / 服务端 API / 即时通信 > 会话管理 > 群管理 > 更新群会话
...
| owner | String | 否 | 群主的userId，可通过[根据手机号查询用户](0062-query-users-by-phone-number.md)接口获取userId。  **[!NOTE]**    该员工必须为会话useridlist的成员之一。 |
...
| add\_extidlist | Array of String | 否 | unionid列表，可通过[查询用户详情](0062-query-users-by-phone-number.md)接口获取。 |
| del\_extidlist | Array of String | 否 | unionId列表，可通过[查询用户详情](0062-query-users-by-phone-number.md)接口获取。 |
| icon | String | 否 | 群头像的mediaId，可通过[上传媒体文件](0645-upload-media-files.md)接口获取media\_id参数值。 |
