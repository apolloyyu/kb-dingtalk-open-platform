...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > OA审批 > 撤销审批实例
...
> - 未接入的开发者建议使用新版[撤销审批实例](0498-revoke-an-approval-instance.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | be311xxxx | 调用服务端API的应用凭证，可通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 |
...
| process\_instance\_id | String | 是 | a171de6c-8bxxxx | 审批实例ID，调用[获取审批实例ID列表](1530-operation-to-retrieve-a-list-of.md)接口获取。 |
