...
- `data`（object）：事件体data。
- `data.projectInfo`（object）：项目数据。
- `data.projectInfo.userDefineCode`（string，必填）：用户自定义code。
- `data.projectInfo.corpId`（string，必填）：企业corpId信息。
- `data.projectInfo.createTime`（long，必填）：创建时间。
- `data.projectInfo.code`（string，必填）：项目code。
- `data.projectInfo.name`（string，必填）：项目名称。
- `data.projectInfo.description`（string，必填）：项目描述。
...
- `biz_data.projectInfo`（object）：项目数据。
- `biz_data.projectInfo.userDefineCode`（string，必填）：用户自定义code。
- `biz_data.projectInfo.corpId`（string，必填）：企业corpId信息。
- `biz_data.projectInfo.createTime`（long，必填）：创建时间。
- `biz_data.projectInfo.code`（string，必填）：项目code。
- `biz_data.projectInfo.name`（string，必填）：项目名称。
- `biz_data.projectInfo.description`（string，必填）：项目描述。
