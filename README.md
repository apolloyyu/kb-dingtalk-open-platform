...
| 快照日期 | 2026-09-04（权威值见 [meta/source_manifest.json](meta/source_manifest.json) 的 crawl_time） |
...
- 流程、跨阶段和多步骤问题会展开 2–3 篇代表正文（每篇 ≤3500 字符），例如教程、凭证获取和身份换取，避免从单一接口外推完整流程。
- 查询里的字段名/JSAPI 名/错误码等 ASCII 标识符会查 `graph/terms.json`（`build_term_index.py` 从正文确定性生成的倒排索引）直达正文，标题层打分按 IDF 加权，避免泛词（如「小程序」）压过罕见词（如「导航栏」）。
...
- 静态快照，时效止于 2026-09-04；时效敏感问题（计费/灰度/上线时间）以线上为准。
