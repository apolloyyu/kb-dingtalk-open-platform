...
调用[注册卡片回调地址](../../01-应用开发/02-4a8AMF6u2A-服务端-API/0787-register-card-callback-address.md)接口，注册动态数据源的回调地址。
...
调用[创建卡片](../../01-应用开发/02-4a8AMF6u2A-服务端-API/0781-interface-for-creating-a-card-instance.md)接口，在`openDynamicDataConfig`参数中配置动态数据源。在[开放接口创建卡片实例](0004-open-the-interface-to-create-a-card-instance.md)中，动态数据源相关配置参数`openDynamicDataConfig`，通过简单实例，针对`openDynamicDataConfig`参数的解释说明：
...
如[注册卡片回调地址](../../01-应用开发/02-4a8AMF6u2A-服务端-API/0787-register-card-callback-address.md)时提供了"卡片数据回调 apiSecret"，钉钉会在请求 Header 中包含签名：
