...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 协同办公 > 日程 > 针对单个日程进行签到
...
> - 未接入的开发者建议使用新版[针对单个日程进行签到](0269-sign-in-single-schedule-news.md)接口，已接入用户不受影响。
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| x-acs-dingtalk-access-token | String | 是 | 调用该接口的访问凭证。   - 企业内部应用可通过[获取企业内部应用的accessToken](0032-obtain-the-access-token-of-an-internal-app.md)接口获取 - 第三方企业应用可通过[获取第三方应用授权企业的accessToken](0033-obtain-the-access-token-of-the-authorized-enterprise-1.md)接口获取。 - 第三方个人应用可调用[获取用户token](0031-obtain-user-token.md)接口获取。 |
