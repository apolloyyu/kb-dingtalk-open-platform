...
> Path: 应用开发 / 服务端 API / 通讯录管理 > 角色管理 > 批量增加员工角色
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| roleIds | String | 是 | 1507113584 | 角色roleId列表，可调用[获取角色列表](0088-obtains-a-list-of-enterprise-roles.md)接口获取。  多个roleId用英文逗号（,）分隔，最多可传20个。 |
| userIds | String | 是 | user1 | 员工的userId，可通过调用[根据手机号查询用户](0062-query-users-by-phone-number.md)获取。  多个userId用英文逗号（,）分隔，最多可传20个。 |
