...
> Path: 应用开发 / 服务端 API / OA 审批 > 自有 OA 审批 > 审批实例 > 创建实例
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| processCode | String | 是 | 审批模板code，可通过调用[获取模板code](0510-obtain-the-template-code.md)接口获取processCode参数值。 |
| originatorUserId | String | 是 | 审批实例发起人的userId。 |
| formComponentValueList | Array | 否 | 表单控件列表，详情请参考[FormComponentValues 参数说明](0473-oa-formcomponent-message.md#9bcb6b14dbz03)说明，最多元素个数：100。      该接口不支持**地点控件**、**电话控件**。 |
