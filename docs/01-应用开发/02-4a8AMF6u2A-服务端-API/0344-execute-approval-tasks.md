...
> Path: 应用开发 / 服务端 API / 宜搭 > 任务 > 同意或拒绝宜搭审批任务
...
| processInstanceId | String | 是 | 实例ID，调用[获取多个表单实例ID](0323-api-searchformdataidlist-v2.md)接口获取。 |
| userId | String | 是 | 用户的userid，可调用[获取部门用户userid列表](0064-query-the-list-of-department-userids.md)接口获取。 |
| taskId | Long | 是 | 任务ID，可通过[获取实例ID列表](0309-api-getinstanceidlist-v2.md)接口获取返回值中的taskId。 |
