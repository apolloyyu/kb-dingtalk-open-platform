...
> Path: 应用开发 / 服务端 API / 更多开放 > 智能硬件 > 设备管理 > 查询设备详情
...
| device\_name | String | 否 | 产品智能 | 设备名称，可通过[查询设备列表](1318-intelligent-hardware-list-query.md)接口获取。  **[!NOTE]**  该参数不能和`device_id`同时为空。 |
| device\_id | String | 否 | QWR45GT | 设备id，可通过[查询设备列表](1318-intelligent-hardware-list-query.md)接口获取。  **[!NOTE]**  该参数不能和`device_name`同时为空。 |
