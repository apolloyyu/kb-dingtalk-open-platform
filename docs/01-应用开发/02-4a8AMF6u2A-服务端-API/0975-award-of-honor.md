...
> Path: 应用开发 / 服务端 API / 企业文化 > 荣誉 > 给员工颁发荣誉
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| honorId | String | 是 | 荣誉Id，可调用[查询当前企业下可颁发的荣誉列表](0977-query-the-list-of-honors-that-can-be-issued-under.md)接口获取honorId参数值。 |
...
| openConversationIds | Array of String | 否 | 接收荣誉消息的群openConversationId列表，最大值。   - 企业内部应用，可调用[创建群会话](0739-create-common-group-new-version-v2.md)接口获取openConversationId。 - 第三方企业应用，可调用[创建场景群](0747-create-a-scene-group.md)接口获取openConversationId。       荣誉发送人senderUserId，必须是群成员。 |
