...
| receiverId | String | 否 | 钉外账号在业务系统内的唯一标志，调用[创建钉钉客联钉外账号](1848-create-bc-account-association.md)接口获取，长度限制为1～64个字符。  **[!NOTE]**  单聊场景必填，可实现钉内账号向钉外账号发送单聊消息。 |
| openConversationId | String | 否 | 群会话openConversationId，可调用[创建钉钉客联普通互通群](1849-create-common-group-new-version.md) / [创建钉钉客联两人互通群](1850-creating-two-groups-of-people.md)接口获取，长度限制为1～32个字符。  **[!NOTE]**  群聊场景必填，可实现钉内账号向互通群内发送群聊信息。 |
