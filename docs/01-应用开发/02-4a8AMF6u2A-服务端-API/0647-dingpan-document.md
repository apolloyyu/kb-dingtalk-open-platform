...
> Path: 应用开发 / 服务端 API / 文档/文件 > 存储管理 > 使用教程 > 钉盘文件的上传、预览和下载
...
2. 钉盘文件上传
   ...
   1. 调用[搜索文件](0637-search-for-files.md)接口，搜索钉盘空间中的名为“测试文件夹”，并返回文件的 `dentryUuid`。
...
4. 钉盘文件下载
   ...
   1. 通过调用[获取空间列表](0635-queries-a-space-list.md)接口，获取钉盘空间信息，获取`spaceId`。
   2. 通过 spaceId 调用[获取文件或文件夹列表](0665-get-a-list-of-files-or-folders.md)接口，获取文件的 `fileId` 和文件名称 `name`。
