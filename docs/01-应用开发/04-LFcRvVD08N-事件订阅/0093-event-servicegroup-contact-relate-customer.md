...
- `data.spiCrmModel.formData`（object，必填）："formData": { // 此事件固定只会有belong\_customer\_id
- `data.spiCrmModel.formData.belongCustomerId`（string）：加密的归属客户数据实例id
...
- `spiCrmModel.formData`（object，必填）："formData": { // 此事件固定只会有belong\_customer\_id
- `spiCrmModel.formData.belong_customer_id`（string，必填）：加密的归属客户数据实例id
