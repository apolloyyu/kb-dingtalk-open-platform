...
archived: true
method: —
endpoint: https://oapi.dingtalk.com/topapi/kac/datav/dept/telconf/list
api_version: v1-oapi
...
## Body
- request (TelConferenceSummaryRequest, required): 请求对象。
- data_id (String, required): 日期标识。
- size (Number, required): 分页大小，不超过100。
- cursor (Number, required): 分页游标。首页请使用0，之后直接使用返回结果中next_cursor的值。

## Returns
- optional: request_id(String), errcode(Number), errmsg(String), result(TelConferenceDeptSummaryResponse), data(TelConferenceDeptSummaryVo[]), dept_id(Number), dept_name(String), join_count(Number), start_avg_len_min(String), start_count(Number), start_len_min(String), has_more(Boolean), next_cursor(Number)
...
updated_at: 2026-08-27 14:08:08
