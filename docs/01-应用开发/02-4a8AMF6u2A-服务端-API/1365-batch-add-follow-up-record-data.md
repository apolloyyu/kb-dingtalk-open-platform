...
> Path: 应用开发 / 服务端 API / 更多开放 > 客户管理（官方CRM） > 跟进记录 > 批量新增跟进记录数据
...
| key | String | 是 | 模型字段key，该参数传跟进记录对象元数据信息中获取字段的name值，调用[获取跟进记录对象的元数据](1364-obtains-the-metadata-description-of-the-crm-follow-up-record-object.md)接口获取name参数值。  该参数是否必填，取决于调用获取跟进记录对象的元数据信息接口，返回的nillable字段值：   - 若nillable是**true**：本接口参数key和value为非必填。 - 若nillable是**false**：本接口参数key和value为必填。 |
