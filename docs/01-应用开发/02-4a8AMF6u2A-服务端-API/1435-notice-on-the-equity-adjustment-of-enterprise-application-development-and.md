...
> Path: 应用开发 / 服务端 API / 平台公告与计费 > 平台公告 > 关于企业应用开发和自动化在钉钉PaaS 底座中的权益调整的通告
...
| **OpenAPI名称** | **OpenAPI详情** |
| --- | --- |
| [添加待入职员工](0944-add-employees-to-be-hired-supports-system-and-custom-fields.md) | 在企业自有通讯录系统内，添加待入职员工时，调用本接口在钉钉智能人事应用内同步添加待入职员工 |
| [配置考勤排班附加信息](0210-synchronization-scheduling-information.md) | 企业考勤排班信息较多，手动配置排班的打卡位置、打卡WiFi信息比较繁琐，可调用本接口，更新排班的附加信息 |
| [查询指定用户的封账规则](0241-encapsulate-account-sealing-and-unsealing-rules.md) | 开启封账后，封账范围内的考勤结果将封存不允许修改。包含以下操作：请假，外出，出差，加班，补卡申请；员工排班，修改考勤结果等，调用本接口根据企业员工userId列表，获取员工的封账规则信息 |
...
| [上传打卡记录](0196-upload-punch-records.md) | 企业使用三方打卡设备或门禁系统刷卡，调用本接口，可将三方打卡或刷卡记录上传到钉钉考勤，作为员工的考勤打卡信息 |
