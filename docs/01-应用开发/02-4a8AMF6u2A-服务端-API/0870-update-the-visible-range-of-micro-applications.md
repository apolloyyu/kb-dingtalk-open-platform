...
| addRoleIds | Array of Long | 否 | 待添加的角色ID列表，最大长度100。可通过[获取角色列表](0088-obtains-a-list-of-enterprise-roles.md)接口获取具体ID值。      添加后总角色数不得超过2000，否则接口会报错。 |
| delRoleIds | Array of Long | 否 | 删除的可使用角色列表，最大长度100。可通过[获取角色列表](0088-obtains-a-list-of-enterprise-roles.md)接口获取id参数值。 |
