...
> Path: 应用开发 / 服务端 API / 认证与授权 > 访问凭证 > 应用身份凭证 > 获取第三方应用授权企业的accessToken
...
| authCorpId | String | 是 | 授权企业的CorpId。   - 网页应用（H5）：  你可以在应用首页地址/PC端首页地址添加参数`corpid=$CORPID$`，例如：https://example.com?corpid=$CORPID$，当从工作台访问该应用时，会将 $CORPID$ 自动解析为当前访问用户所在的组织 ID。   image.png - 小程序：  使用客户端[corpId](../03-Ogu5SlPY4t-客户端-JSAPI/0025-jsapi-corp-id.md)接口获取。 |
