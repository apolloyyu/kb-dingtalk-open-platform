...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > OA审批 > 创建实例
...
> - 未接入的开发者建议使用新版[创建实例](0512-create-a-ticket-approval-instance.md)接口，已接入用户不受影响。

## **接口说明**

- 调用该接口创建实例，接口返回的**审批实例ID**请务必注意保存，方便后续调用其他接口使用。
- 创建实例后，根据**审批实例ID**调用服务端API-[创建待办事项](1520-create-a-to-do-task.md)接口，可以在钉钉客户端待办中查看审批实例。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6d1bxxxx | 调用该接口的应用凭证。   - 企业内部应用，通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 - 第三方企业应用，通过[服务商获取第三方应用授权企业的access\_token](1445-obtain-isvapp-token.md)接口获取。 |
...
| agentid | Number | 是 | 1234 | 应用标识。可在[开发者后台](https://open-dev.dingtalk.com/)的应用详情页获取。应用的agentid。   - 企业内部应用可在[开发者后台](https://open-dev.dingtalk.com/)的应用详情页获取。  image - 第三方企业应用可调用[获取企业授权信息](0041-obtains-the-basic-information-of-an-enterprise.md)接口获取   **[!IMPORTANT]**  如果是第三方企业应用必须指定该参数。 |
| process\_code | String | 是 | PROC-EF6YJL35P2xxxx | 审批模板唯一码，调用[创建或更新审批模板](1529-save-approval-template.md)接口获取process\_code参数值。 |
...
| 8100017 | 无操作审批流的权限，请检查审批实例或者模板是否正确 | processCode参数不正确 | processCode必须使用[创建或更新审批模板](1529-save-approval-template.md)接口返回的processCode，并且参数fake\_mode必须传true。 |
