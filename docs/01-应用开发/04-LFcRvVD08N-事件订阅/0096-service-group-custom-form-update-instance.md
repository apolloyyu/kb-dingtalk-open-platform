...
- `data.spiCrmModel.formData.belongCustomerId`（string）：加密的所属客户id
- `data.spiCrmModel.operatorUnionId`（string）：加密的操作者unonId。
...
- `spiCrmModel.formData.belong_customer_id`（string）：加密的所属客户id
- `spiCrmModel.operatorUnionId`（string）：加密的操作者unonId。
