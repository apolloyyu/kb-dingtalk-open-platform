...
步骤五，根据用户个人authCode，获取个人应用访问凭证[获取用户token](0031-obtain-user-token.md)。

步骤六，调用服务端数字化管理师相关API。

1. 调用服务端API-[检查用户是否完成所有任务](1336-docking-of-provincial-practical-exercises-for-digital-managers.md)接口，检查用户是否完成所有任务。
...
根据AppId、AppSecret和用户个人authCode，获取个人应用访问凭证[获取用户token](0031-obtain-user-token.md)。
