...
> Path: 应用开发 / 服务端 API / 通讯录管理 > 用户管理 > 获取部门用户详情
...
- 调用本接口可以实现获取部门普通账号用户详情或获取部门企业账号用户详情。由于在调用时参数使用有较多区别，为便于开发者查看，按照新用户的账号类型进行拆分优化：
  ...
  - 如果是获取部门企业账号用户详情（一般是企业钉钉用户），接口说明文档请参见[获取部门企业账号用户详情](0115-queries-account-details.md)。
...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| dept\_id | Number | 是 | 10 | 部门ID，可调用[获取部门列表](0081-user-management-acquires-the-list-departments.md)获取，如果是根部门，该参数传1。  **[!NOTE]**  只获取当前部门下的员工信息，不包含子部门内的员工。 |
...
| exclusive\_account | Boolean | false | 是否企业账号：   - **true**：是 - **false**：不是   **[!NOTE]**   - 企业账号介绍请参见[企业账号概述](0102-dedicated-account-overview.md)。 - 第三方企业应用不返回该参数。 |
