...
> Path: 应用开发 / 服务端 API / OA 审批 > 官方 OA 审批 > 审批钉盘空间&附件 > 授权下载审批钉盘文件
...
| fileId | String | 是 | 文件fileId，调用[获取单个审批实例详情](0497-obtains-the-details-of-a-single-approval-instance-pop.md)接口获取`fileId`参数值。      文件id是审批组件中上传的fileId。 |
| spaceId | Long | 是 | 审批钉盘空间spaceId，可调用[获取审批钉盘空间信息](0501-obtains-the-information-about-approval-nail-disk.md)接口获取`spaceId`参数值。 |
