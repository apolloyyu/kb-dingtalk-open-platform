...
1. 获取审批钉盘空间spaceId，调用[获取审批钉盘空间信息](0501-obtains-the-information-about-approval-nail-disk.md)接口，获取审批钉盘空间spaceId。
...
3. 获取审批实例processInstanceId，调用[发起审批实例](0496-create-an-approval-instance.md)接口获取InstanceId参数值。
4. 根据上述获取信息，调用本接口，授权用户审批附件预览权限。每一次预览审批附件前，都需要调用本接口进行授权。
5. 调用[预览钉盘文件](../03-Ogu5SlPY4t-客户端-JSAPI/0331-jsapi-preview-file-in-ding-talk.md)接口，完成预览操作。
...
| agentId | Long | 否 | 应用的agentId。   - 企业内部应用，可查看[基础信息-AgentId](https://open.dingtalk.com/document/development/basic-concepts-beta#884d363067bnq)信息。 - 第三方企业应用，可调用[获取企业授权信息](0041-obtains-the-basic-information-of-an-enterprise.md)接口获取`agentid`参数值。 |
| processInstanceId | String | 是 | 审批实例ID。   - 调用[发起审批实例](0496-create-an-approval-instance.md)接口获取`InstanceId`参数值。 - 调用[获取审批实例ID列表](0500-obtain-an-approval-list-of-instance-ids.md)接口获取`list`参数值。 |
