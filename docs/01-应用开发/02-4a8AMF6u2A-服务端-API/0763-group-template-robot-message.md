...
| receiver\_union\_ids | String | 否 | "jHs\*\*\*\*\*,atc\*\*\*\*\*" | 消息接收人unionId列表，调用[查询用户详情](0055-query-user-details.md)接口获取`unionid`参数值。  **[!NOTE]**  不设置任何接收人则消息对群内所有成员可见。 |
| receiver\_mobiles | String | 否 | "137\*\*\*\*\*000,158\*\*\*\*\*000" | 消息接收人手机号列表，调用[查询用户详情](0055-query-user-details.md)接口获取`mobile`参数值。  **[!NOTE]**  不设置任何接收人则消息对群内所有成员可见。 |
| at\_mobiles | String | 否 | "137\*\*\*\*\*000" | @人的手机号列表，调用[查询用户详情](0055-query-user-details.md)接口获取`mobile`参数值。  **[!NOTE]**  一次调用最多支持50人。 |
