...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > e签宝 1.0 > 文件 > 获取文件上传地址
...
> - 未接入的开发者建议使用新版[获取文件上传地址](1082-obtain-the-upload-url-of-a-file-1.md)接口，已接入用户不受影响。
...
| data | Object | 返回结果。 |
| fileId | String | 文件Id。 |
| uploadUrl | String | 文件直传地址。在获取到文件上传地址后，可以直接使用此地址进行文件上传，详情请参考[上传文件](1504-upload-file-stream.md)。      可以重复使用，但是只能传一样的文件，有效期一小时。 |
