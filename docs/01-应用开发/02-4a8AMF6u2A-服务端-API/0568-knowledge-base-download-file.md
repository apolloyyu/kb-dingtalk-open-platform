...
> Path: 应用开发 / 服务端 API / 文档/文件 > 知识库 > 知识库目录树管理 > 知识库下载文件
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| spaceId | String | 是 | 空间Id，可通过[获取节点列表](0570-get-node-list.md)接口获取知识库节点的 `dentryUuid`，再调用[根据 dentryUuid 获取 spaceId](0672-api-getdentryidbyuuid.md)接口获取 `spaceId`。 |
| dentryId | String | 是 | 文件Id，可通过[获取节点列表](0570-get-node-list.md)接口获取知识库节点的 `dentryUuid`，再调用[根据 dentryUuid 获取 spaceId](0672-api-getdentryidbyuuid.md)接口获取 `dentryId`。 |

### 查询参数

| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| unionId | String | 是 | 操作者unionId，可调用[查询用户详情](0055-query-user-details.md)接口获取。 |
...
| version | Long | 否 | 文件的版本号，可调用[获取文件版本列表](0662-obtains-a-list-of-file-versions.md)接口获取version。 |
