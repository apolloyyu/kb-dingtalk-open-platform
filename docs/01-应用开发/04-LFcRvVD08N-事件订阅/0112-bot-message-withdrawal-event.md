...
- `data`（object）：事件体data。
- `data.operatTime`（long）：消息撤回时间。
- `data.unionId`（string）：单聊时，消息被撤回人的unionId。
- `data.userId`（string）：单聊时，消息被撤回人的userId，如果用户属于corpId对应企业会透出此字段。
...
- `eventId`（String）：事件的唯一Id。
- `OperatTime`（long）：消息撤回时间。
- `UnionId`（string）：单聊时，消息被撤回人的unionId。
- `UserId`（string）：单聊时，消息被撤回人的userId，如果用户属于corpId对应企业会透出此字段。
...
- `biz_data.OperatTime`（long）：消息撤回时间。
- `biz_data.UnionId`（string）：单聊时，消息被撤回人的unionId。
- `biz_data.UserId`（string）：单聊时，消息被撤回人的userId，如果用户属于corpId对应企业会透出此字段。
