...
4. 调用通讯录相关API：
   ...
   1. 调用服务端API-[创建部门](0076-address-book-creation-department-established-department.md)接口，实现创建部门，获取部门`dept_id`**。**
      ...
      - 如果创建的是其他部门的子部门，需要先调用[获取部门列表](0081-user-management-acquires-the-list-departments.md)接口，获取的部门ID作为parent\_id的值。
   2. 根据部门`dept_id`，调用服务端API-[获取部门详情](0080-query-department-details0-v2.md)接口，获取部门详情信息。
   3. 根据部门`dept_id`，调用服务端API-[更新部门](0077-address-book-update-department.md)接口，实现更新部门信息。
   4. 根据部门`dept_id`，调用服务端API-[删除部门](0078-address-book-deletion-department.md)接口，实现删除部门信息。
...
调用服务端API-[创建部门](0076-address-book-creation-department-established-department.md)接口，实现部门创建并获取唯一标识`dept_id`：
...
- **parentId：**父部门ID，决定部门在组织架构树中的位置。
  ...
  - 根部门的子部门：`parentId=1`
  - 其他部门的子部门：需先调用[获取部门列表](0081-user-management-acquires-the-list-departments.md)接口获取对应部门的`dept_id`
...
- **深层嵌套部门**：先调用[获取部门列表](0081-user-management-acquires-the-list-departments.md)接口获取父部门ID。
...
根据部门`dept_id`[获取部门详情](0080-query-department-details0-v2.md)，用于数据同步后的校验或业务系统读取：
...
> **[!NOTE]**
>
> 部分字段的返回受权限限制，请参考[获取部门详情](0080-query-department-details0-v2.md)的返回字段说明。
...
根据部门`dept_id`[更新部门](0077-address-book-update-department.md)属性，实现更新部门信息：
...
根据部门`dept_id`从企业通讯录中[删除部门](0078-address-book-deletion-department.md)：
...
- 确认该部门下无在职员工（调用[获取部门详情](0080-query-department-details0-v2.md)接口检查）。
- 确认该部门的所有子部门下也无在职员工。
...
- 调用[删除部门](0078-address-book-deletion-department.md)接口移除空部门。
...
| `errcode=400003` | 部门不存在 | 确认`dept_id`是否正确，或先调用[创建部门](0076-address-book-creation-department-established-department.md)接口。 |
