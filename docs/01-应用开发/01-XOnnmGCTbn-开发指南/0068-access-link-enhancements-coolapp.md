...
4. 开发链接增强回调HTTP接口：
   ...
   2. 业务方解析url等参数，并自行调用[创建卡片](../02-4a8AMF6u2A-服务端-API/0781-interface-for-creating-a-card-instance.md)接口创建卡片实例（注意卡片创建者的userId在此场景需要必填，否则发送者自己可能看不到卡片）。
   3. 业务方响应钉钉侧数据结构（出参）：
...
链接增强酷应用完成发布后，需要完成[发布应用](0019-publish-dingtalk-application.md)。
