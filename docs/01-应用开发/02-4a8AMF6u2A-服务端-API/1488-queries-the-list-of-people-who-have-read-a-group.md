...
| 参数 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| access\_token | String | 是 | 调用服务端API的应用凭证，可通过[获取企业内部应用的access\_token](1443-obtain-orgapp-token.md)接口获取。 |
| messageId | String | 是 | [发送消息到企业群](1487-send-group-messages.md)接口返回的加密消息id。  **[!IMPORTANT]**  消息id中包含url特殊字符时需要encode后再使用。 |
