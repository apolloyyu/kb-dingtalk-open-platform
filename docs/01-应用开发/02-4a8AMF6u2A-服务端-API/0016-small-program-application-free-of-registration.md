...
1. 调用`dd.getAuthCode`接口获取免登授权码，小程序免登可参考[小程序免登](../03-Ogu5SlPY4t-客户端-JSAPI/0006-jsapi-get-auth-code.md)。
   ...
   ```
2. 调用[获取应用的 Access Token](0036-api-gettoken.md)接口，获取应用级别的访问凭证。
3. 根据免登授权码`code`和应用级`access_token`，调用[通过免登码获取用户信息](0023-obtain-the-userid-of-a-user-by-using-the-log-free.md)接口，获取当前用户的`userId`和`unionId`等身份信息。
