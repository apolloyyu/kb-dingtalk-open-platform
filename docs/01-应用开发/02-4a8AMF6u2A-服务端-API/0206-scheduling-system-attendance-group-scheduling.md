...
> Path: 应用开发 / 服务端 API / 考勤 > 考勤排班 > 排班制考勤组排班
...
| group\_id | Number | 是 | 2987 | 考勤组ID。  **[!NOTE]**  如果你使用的是旧考勤组标识即group\_key，可以调用[groupKey转换为groupId](0175-convert-groupkey-to-groupid.md)接口将group\_key转换为group\_id。 |
| schedules | TopScheduleParam[] | 是 |  | 排班详情。示例如下：   ``` {   "is_rest": false,   "work_date": 1605150671000,   "shift_id": 1,   "userid": "user123" } ```   **[!NOTE]**  最大列表长度200。 |
| shift\_id | Number | 是 | 1 | 班次ID，休息班次传1，可通过[获取班次摘要信息](0202-enterprise-shift-query-in-batches.md)接口获取id参数值。  **[!NOTE]**   - 当is\_rest参数传true时，shift\_id传1。 - 如果你需要清空排班，shift\_id传 -2。 |
