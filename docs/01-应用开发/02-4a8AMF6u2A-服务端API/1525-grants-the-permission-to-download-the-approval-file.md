...
- 如果审批单是手动在钉钉客户端发起的，手动选择本地文件作为附件，调用该接口获取的附件下载地址是这样的格式为`#zifgs49xxxxx.file`。
  ...
  需要与[获取单个审批实例详情](1533-get-details-single-approval-instance.md)接口结合使用，获取实例详情接口得到附件的fileName和fileType，按照本接口返回的`#zifgs49xxxxx.file`进行名称和后缀替换，该附件就可以正常打开了。
...
| process\_instance\_id | String | 是 | 123a-234bxx | 审批单实例id，调用[获取单个审批实例详情](1533-get-details-single-approval-instance.md)接口获取。 |
| file\_id | String | 是 | 123456 | 文件id，调用[获取单个审批实例详情](1533-get-details-single-approval-instance.md)接口获取。  **[!IMPORTANT]**  文件id是审批组件中上传的fileid（如下图所示），评论中上传的附件fileid暂不支持获取下载链接。  文件组件 |
