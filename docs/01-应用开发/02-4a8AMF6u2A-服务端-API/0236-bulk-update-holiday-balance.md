...
> Path: 应用开发 / 服务端 API / 考勤 > 假期管理 > 批量更新假期余额
...
1. 调用[添加假期规则](0232-add-holiday-rules.md)接口创建普通假期，再调用[初始化假期余额](0235-initialize-holiday-balance.md)接口初始化余额是8天。
...
| --- | --- |
| 调用[添加假期规则](0232-add-holiday-rules.md)接口创建的假期类型。 | 支持 |
...
| leave\_code | String | 是 | f84a2dxxxx | 自定义添加的假期类型。  **[!NOTE]**  此类型必须是通过[添加假期规则](0232-add-holiday-rules.md)接口创建的假期类型。 |
