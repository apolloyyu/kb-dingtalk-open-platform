...
| 名称 | 类型 | 是否必填 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| access\_token | String | 是 | 6ed1bxxx | 调用服务端API授权凭证，通过[获取企业内部应用的access\_token](1446-obtain-orgapp-token.md)接口获取。 |
| userid | String | 是 | user123 | 员工唯一标识userid，可通过[根据手机号查询用户](1465-retrieve-userid-from-mobile-phone-number.md)接口获取userid。 |
