...
doc_id: "wyAs6hk8Le"
updated_at: "2026-08-27 14:22:34"
---

> Source: https://open.dingtalk.com/document/development/create-bc-account-association
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 钉钉客联 > 创建钉钉客联钉外账号
> Updated: 2026-08-27 14:22:34

# 创建钉钉客联钉外账号

在互通场景中，需要有钉外账号，通过该接口创建钉外用户账号，并且可以建立与钉内用户账号的关联关系，方便后续建群等操作。
...
| appUserId | String | 是 | 钉外账号在业务系统内的唯一标志，长度限制为1～64个字符。 |
| appUserName | String | 是 | 钉外账号名称，长度限制为1~64个字符。 |
| appUserAvatarMediaType | Integer | 否 | 钉外账号头像类型，取值：   - **1**：http类型 |
| appUserAvatar | String | 否 | 钉外账号头像链接，长度限制为1～1024个字符。 |
| appUserDynamics | String | 否 | 钉外账号动态信息，长度限制为1～64个字符。 例如：认真工作,快乐生活 |
| appUserMobile | String | 是 | 钉外账号手机号，长度限制为11位号码。 |
| userId | String | 否 | 钉内账号userId，长度限制为1～64个字符。 |
| channelCode | String | 是 | 渠道code。  **[!NOTE]**  该参数在创建渠道后获取，如何创建可参考[渠道配置](1839-interconnections-channel.md)文档。 |
