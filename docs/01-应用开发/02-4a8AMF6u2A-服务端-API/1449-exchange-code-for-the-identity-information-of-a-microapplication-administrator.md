...
> Path: 应用开发 / 服务端 API / 历史文档（不推荐） > 身份验证（免登） > 获取应用管理后台免登的用户信息
...
> - 未接入的开发者建议使用新版 [获取应用管理后台免登的用户信息](0025-obtains-the-identity-of-an-application-administrator.md)接口，已接入用户不受影响。
...
| access\_token | String | 是 | 6ed1bxxx | 调用该API的应用凭证，可调用[获取微应用后台免登的accessToken](0024-obtain-the-access-token-of-the-micro-application-background-without-log-on.md)接口获取。 |
